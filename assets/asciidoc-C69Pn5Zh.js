...
var require_asciidoc_highlight_rules = /* @__PURE__ */ __commonJSMin(((exports) => {
	var oop$2 = require_oop();
	var TextHighlightRules = require_text_highlight_rules().TextHighlightRules;
	var AsciidocHighlightRules$1 = function() {
		...
		function quoteRule(ch) {
			return (/\w/.test(ch) ? "\\b" : "(?:\\B|^)") + ch + "[^" + ch + "].*?" + ch + "(?![\\w*])";
		}
		var tokenMap = {
		...
		for (var state in this.$rules) {
			...
			for (var i = stateRules.length; i--;) {
				...
				} else if (rule.token in tokenMap) rule.token = tokenMap[rule.token];
			}
		}
	};
	oop$2.inherits(AsciidocHighlightRules$1, TextHighlightRules);
	exports.AsciidocHighlightRules = AsciidocHighlightRules$1;
}));
var require_asciidoc$1 = /* @__PURE__ */ __commonJSMin(((exports) => {
	...
	(function() {
		...
		this.getFoldWidgetRange = function(session, foldStyle, row) {
			...
			if (getTokenType(row) == heading) {
				...
				endRow = token && token.value.match(this.singleLineHeadingRe) ? row - 1 : row - 2;
				if (endRow > startRow) while (endRow > startRow && (!getTokenType(endRow) || token.value[0] == "[")) endRow--;
				if (endRow > startRow) {
					var endColumn = session.getLine(endRow).length;
					return new Range(startRow, startColumn, endRow, endColumn);
				}
			} else if (session.bgTokenizer.getState(row) == "dissallowDelimitedBlock") {
				while (row-- > 0) if (session.bgTokenizer.getState(row).lastIndexOf("Block") == -1) break;
				endRow = row + 1;
				if (endRow < startRow) {
				...
				while (++row < maxRow) if (session.bgTokenizer.getState(row) == "dissallowDelimitedBlock") break;
				endRow = row;
				if (endRow > startRow) {
					var endColumn = session.getLine(row).length;
...
var require_asciidoc = /* @__PURE__ */ __commonJSMin(((exports) => {
	var oop = require_oop();
	var TextMode = require_text().Mode;
	var AsciidocHighlightRules = require_asciidoc_highlight_rules().AsciidocHighlightRules;
	var AsciidocFoldMode = require_asciidoc$1().FoldMode;
	var Mode = function() {
	...
	(function() {
		...
		this.getNextLineIndent = function(state, line, tab) {
			if (state == "listblock") {
				var match = /^((?:.+)?)([-+*][ ]+)/.exec(line);
				if (match) return new Array(match[1].length + 1).join(" ") + match[2];
				else return "";

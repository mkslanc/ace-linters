...
var require_vbscript_highlight_rules = /* @__PURE__ */ __commonJSMin(((exports) => {
	var oop$2 = require_oop();
	var TextHighlightRules = require_text_highlight_rules().TextHighlightRules;
	var VBScriptHighlightRules$1 = function() {
	...
	oop$2.inherits(VBScriptHighlightRules$1, TextHighlightRules);
	exports.VBScriptHighlightRules = VBScriptHighlightRules$1;
}));
var require_vbscript$1 = /* @__PURE__ */ __commonJSMin(((exports) => {
	...
	(function() {
		...
		this.vbsBlock = function(session, row, column, tokenRange) {
			...
			if (doubleKeywordResult === "ignore") return;
			...
			switch (val) {
				...
				case "with":
					...
					if (!checkToken.test(line) && !endTest) return;
					if (endTest) {
						...
						if (token) {
							...
							if (val == "end") {
								...
								firstRange = new Range$1(firstRange.start.row, firstRange.start.column, tokenRange.start.row, tokenRange.end.column);
							}
						}
						dir = -1;
					}
				...
				case "end":
					...
					if (token) {
						...
						if (val in endOpenings) {
							...
							firstRange = new Range$1(tokenPos.row, tokenPos.column, nextTokenPos.row, endColumn);
						}
					}
					stream.step = stream.stepBackward;
					stream.step();
			...
			while (token = stream.step()) {
				...
				switch (val) {
					...
					case "else":
						level = 0;
						if (startTokenValue != "elseif") ignore = true;
						break;
				}
								...
								if (token) {
									...
									if (val in endOpenings) {
										if (startTokenValue == "else" || startTokenValue == "elseif") {
											if (val !== "if") ranges.shift();
										} else if (val != startTokenValue) ranges.shift();
										var nextTokenPos = stream.getCurrentTokenPosition();
										var endColumn = nextTokenPos.column + val.length;
			...
			if (tokenRange) {
				if (!outputRange) ranges.push(stream.getCurrentTokenRange());
				else ranges.push(outputRange);
				return ranges;
			}
		...
		this.$isDoubleKeyword = function(currentToken, stream) {
			...
			if (prevKeyword) {
				...
				if ((val === "while" || val === "until") && prevVal === "do") return {
					range: this.$getDoubleKeywordRange(prevKeywordIndex, tokenIndex, stream),
					position: "second",
					keyword: "do"
				};
				if ((val === "while" || val === "until") && prevVal === "loop") return {
					range: this.$getDoubleKeywordRange(prevKeywordIndex, tokenIndex, stream),
					position: "second",
					keyword: "loop"
				};
				if (prevVal === "exit" && (val === "for" || val === "do" || val === "sub" || val === "function" || val === "property")) return "ignore";
			}
			...
			if (nextKeyword) {
				...
				if (val === "do" && (nextVal === "while" || nextVal === "until")) return {
					range: this.$getDoubleKeywordRange(tokenIndex, nextKeywordIndex, stream),
					position: "first",
					keyword: "do"
				};
				if (val === "loop" && (nextVal === "while" || nextVal === "until")) return {
					range: this.$getDoubleKeywordRange(tokenIndex, nextKeywordIndex, stream),
					position: "first",
					keyword: "loop"
				};
				if (val === "exit" && (nextVal === "for" || nextVal === "do" || nextVal === "sub" || nextVal === "function" || nextVal === "property")) return "ignore";
			}
			return false;
		};
		this.$getDoubleKeywordRange = function(firstTokenIndex, secondTokenIndex, stream) {
			...
			var rowTokens = stream.$rowTokens;
			var firstStart = 0;
			for (var i = 0; i < firstTokenIndex; i++) firstStart += rowTokens[i].value.length;
			var secondEnd = 0;
			for (var i = 0; i <= secondTokenIndex; i++) secondEnd += rowTokens[i].value.length;
			return new Range$1(row, firstStart, row, secondEnd);
		};
	}).call(FoldMode$1.prototype);
}));
var require_vbscript = /* @__PURE__ */ __commonJSMin(((exports) => {
	var oop = require_oop();
	var TextMode = require_text().Mode;
	var VBScriptHighlightRules = require_vbscript_highlight_rules().VBScriptHighlightRules;
	var FoldMode = require_vbscript$1().FoldMode;
	...
	(function() {
		...
		function isSecondOfDoubleKeyword(tokens, i) {
			...
			if ((val === "while" || val === "until") && (prevVal === "do" || prevVal === "loop")) return true;
			if (prevVal === "exit" && (val === "for" || val === "do" || val === "sub" || val === "function" || val === "property")) return true;
			return false;
		}
			...
			for (var i = 0; i < tokens.length; i++) {
				...
				if (token.type == "keyword.control.asp" || token.type == "storage.type.function.asp") {
					...
					if (val in indentKeywords) {
						if (isSecondOfDoubleKeyword(tokens, i)) continue;
						switch (val) {
							case "property":
							case "sub":
							case "function":
							...
							case "if":
								var checkToken = new RegExp("^\\s* end\\s+" + val, "i");
								if (!/^\s*If\s+.*\s+Then(?!')\s+(?!')\S/i.test(line) && !checkToken.test(line)) level += indentKeywords[val];
								break;
							default:
		...
		this.getNextLineIndent = function(state, line, tab) {
			...
			if (state == "start") level = getNetIndentLevel(tokens, line, this.indentKeywords);

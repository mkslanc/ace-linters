...
import { r as require_search } from "./editor-BiOsjB7l.js";
import { i as require_search_highlight } from "./edit_session-CDHRvoey.js";
import "./tooltip-DAauyLxM.js";
import "./tokenizer-C2b-GJMk.js";
import { t as require_text } from "./text-B9A1mx6l.js";
import "./token_iterator-CbfYmntj.js";
import "./hash_handler-G_6vQiwI.js";
import { n as require_multi_select_commands, t as require_multi_select } from "./multi_select-B30HHNMb.js";
import { t as require_hardwrap } from "./hardwrap-CBWtaseQ.js";
var require_vim = /* @__PURE__ */ __commonJSMin(((exports) => {
	...
	CodeMirror.lookupKey = function lookupKey$1(key, map, handle) {
		if (!map) map = "default";
		...
		if (map.fallthrough) {
			if (!Array.isArray(map.fallthrough)) return lookupKey$1(key, map.fallthrough, handle);
			for (var i = 0; i < map.fallthrough.length; i++) {
				var result = lookupKey$1(key, map.fallthrough[i], handle);
	...
	(function() {
		...
		this.onChange = function(delta) {
			if (this.$lineHandleChanges) this.$lineHandleChanges.push(delta);
		...
		this.onSelectionChange = function() {
			...
			if (this.ace.inMultiSelectMode) this.ace.keyBinding.removeKeyboardHandler(multiSelectCommands.keyboardHandler);
		};
		this.operation = function(fn, force) {
			if (!force && this.curOp || force && this.curOp && this.curOp.force) return fn();
			if (force || !this.ace.curOp) {
				if (this.curOp) this.onBeforeEndOperation();
			}
			if (!this.ace.curOp) {
				var prevOp = this.ace.prevOp;
				this.ace.startOperation({ command: {
					name: "vim",
		...
		this.getCursor = function(p) {
			...
			return toCmPos(p == "anchor" ? sel.isEmpty() ? sel.lead : sel.anchor : p == "head" || !p ? sel.lead : sel.getRange()[p]);
		};
		...
		this.setSelections = function(p, primIndex) {
			...
			if (!primIndex) ranges = ranges.reverse();
			else if (ranges[primIndex]) ranges.push(ranges.splice(primIndex, 1)[0]);
		...
		this.setSelection = function(a, h, options$1) {
			var sel = this.ace.selection;
			sel.moveTo(a.line, a.ch);
			sel.selectTo(h.line, h.ch);
			if (options$1 && options$1.origin == "*mouse") this.onBeforeEndOperation();
		};
		this.somethingSelected = function(p) {
			return !this.ace.selection.isEmpty();
		};
		this.clipPos = function(p) {
			return toCmPos(this.ace.session.$clipPositionToDocument(p.line, p.ch));
		};
		...
		this.$updateMarkers = function(delta) {
			...
			for (var i in this.marks) {
				...
				if (cmp < 0) continue;
				if (cmp === 0) {
					if (isInsert) if (!point.$insertRight) cmp = 1;
					else if (point.bias == 1) cmp = 1;
					else {
						point.bias = -1;
						continue;
					}
		...
		this.setBookmark = function(cursor, options$1) {
			var bm = new Marker(this, this.$uid++, cursor.line, cursor.ch);
			if (!options$1 || !options$1.insertLeft) bm.$insertRight = true;
			this.marks[bm.id] = bm;
			return bm;
		};
		...
		this.findPosV = function(start, amount, unit, goalColumn) {
			if (unit == "page") {
				var config = this.ace.renderer.layerConfig;
			...
			if (unit == "line") {
				...
				return toCmPos(this.ace.session.screenToDocumentPosition(screenPos.row, screenPos.column));
			} else debugger;
		};
		...
		this.coordsChar = function(pos, mode) {
			...
			if (mode == "local") {
				...
				return toCmPos(renderer.session.screenToDocumentPosition(row, col));
			} else if (mode == "div") throw "not implemented";
		};
		this.getSearchCursor = function(query, pos, caseFold) {
			...
			return {
				...
				find: function(back) {
					...
					last = search.find(cm.ace.session);
				...
				replace: function(text) {
					if (last) last.end = cm.ace.session.doc.replace(last, text);
				}
			};
		};
		...
		this.addOverlay = function(o) {
			if (!this.$searchHighlight || !this.$searchHighlight.session) {
				...
				highlight.id = this.ace.session.addDynamicMarker(highlight).id;
				highlight.session = this.ace.session;
				highlight.destroy = function(o$1) {
		...
		this.removeOverlay = function(o) {
			if (this.$searchHighlight && this.$searchHighlight.session) this.$searchHighlight.destroy();
		};
		...
		this.scanForBracket = function(pos, dir, _, options$1) {
			var re = options$1.bracketRegex.source;
			var tokenRe = /paren|text|operator|tag/;
			if (dir == 1) var m = this.ace.session.$findClosingBracket(re.slice(1, 2), toAcePos(pos), tokenRe);
			else {
				var m = this.ace.session.$findOpeningBracket(re.slice(-2, -1), {
					row: pos.line,
					column: pos.ch + 1
				}, tokenRe);
				if (!m && options$1.bracketRegex && options$1.bracketRegex.test(this.getLine(pos.line)[pos.ch - 1])) m = {
					row: pos.line,
					column: pos.ch - 1
				};
			}
		...
		this.getLineNumber = function(handle) {
			...
			for (var i = 0; i < deltas.length; i++) {
				...
				if (delta.start.row != delta.end.row) {
					...
					} else if (delta.start.row < row) {
						if (row < delta.end.row || row == delta.end.row && delta.start.column > 0) return null;
  ...
  font-family: monospace;
	...
	(function() {
		function dialogDiv(cm, template, bottom) {
			var dialog = cm.ace.container.appendChild(document.createElement("div"));
		...
		CodeMirror.defineExtension("openDialog", function(template, callback, options$1) {
			if (this.virtualSelectionMode()) return;
			if (!options$1) options$1 = {};
			closeNotification(this, null);
			var dialog = dialogDiv(this, template, options$1.bottom);
			...
			function close(newVal) {
				if (typeof newVal == "string") inp.value = newVal;
					...
					closed = true;
					dialog.remove();
					if (options$1.onClose) options$1.onClose(dialog);
					var cm = me;
					if (cm.state.vim) {
			...
			if (inp) {
				if (options$1.value) {
					inp.value = options$1.value;
					if (options$1.selectValueOnOpen !== false) inp.select();
				}
				if (options$1.onInput) CodeMirror.on(inp, "input", function(e) {
					options$1.onInput(e, inp.value, close);
				});
				if (options$1.onKeyUp) CodeMirror.on(inp, "keyup", function(e) {
					options$1.onKeyUp(e, inp.value, close);
				});
				CodeMirror.on(inp, "keydown", function(e) {
					if (options$1 && options$1.onKeyDown && options$1.onKeyDown(e, inp.value, close)) return;
					if (e.keyCode == 13) callback(inp.value);
					if (e.keyCode == 27 || options$1.closeOnEnter !== false && e.keyCode == 13) {
						CodeMirror.e_stop(e);
						close();
					}
				});
				if (options$1.closeOnBlur !== false) CodeMirror.on(inp, "blur", close);
				inp.focus();
				...
				if (options$1.closeOnBlur !== false) CodeMirror.on(button, "blur", close);
				button.focus();
			}
			return close;
		});
		CodeMirror.defineExtension("openNotification", function(template, options$1) {
			if (this.virtualSelectionMode()) return;
			closeNotification(this, close);
			var dialog = dialogDiv(this, template, options$1 && options$1.bottom);
			var closed = false, doneTimer;
			var duration = options$1 && typeof options$1.duration !== "undefined" ? options$1.duration : 5e3;
			function close() {
				if (closed) return;
				closed = true;
	...
	function updateSelectionForSurrogateCharacters(cm, curStart, curEnd) {
		if (curStart.line === curEnd.line && curStart.ch >= curEnd.ch - 1) {
			var charCode = cm.getLine(curStart.line).charCodeAt(curStart.ch);
			if (55296 <= charCode && charCode <= 55551) curEnd.ch += 1;
		}
		return {
			start: curStart,
			end: curEnd
	...
	function getOnPasteFn(cm) {
		...
		if (!vim.onPasteFn) vim.onPasteFn = function() {
			if (!vim.insertMode) {
	...
	defineOption("textwidth", 80, "number", ["tw"], function(width, cm) {
		if (cm === void 0) return;
		if (width === void 0) return cm.getOption("textwidth");
		else {
			var column = Math.round(width);
			if (column > 1) cm.setOption("textwidth", column);
		}
	});
	...
	var vimApi = {
		...
		findKey: function(cm, key, origin) {
			...
			function handleKeyInsertMode() {
				...
				if (match.type == "none") {
					...
					lastInsertModeKeyTimer = keysAreChars && window.setTimeout(function() {
						if (vim.insertMode && vim.inputState.keyBuffer.length) clearInputState(cm);
			...
			if (command === false) return !vim.insertMode && key.length === 1 ? function() {
				...
				if ((command.operator || command.isEdit) && cm.getOption("readOnly")) return;
				return cm.operation(function() {
					cm.curOp.isVimOp = true;
					try {
						if (command.type == "keyToKey") doKeyToKey(cm, command.toKeys, command);
	...
	function sendKeyToPrompt(key) {
		if (key[0] == "<") {
			...
			if (lowerKey == "lt") key = "<";
				...
				if (virtualPrompt.onKeyDown) virtualPrompt.onKeyDown(event$1, virtualPrompt.value, close);
				if (virtualPrompt && virtualPrompt.onKeyUp) virtualPrompt.onKeyUp(event$1, virtualPrompt.value, close);
				return;
			}
		}
		if (key == "\n") {
			var prompt$1 = virtualPrompt;
			virtualPrompt = null;
			prompt$1.onClose && prompt$1.onClose(prompt$1.value);
		} else virtualPrompt.value = (virtualPrompt.value || "") + key;
		function close(value$1) {
			if (typeof value$1 == "string") virtualPrompt.value = value$1;
	...
	function doKeyToKey(cm, keys, fromKey) {
		...
		try {
			...
			while (match = keyRe.exec(keys)) {
				...
				if (!vimApi.handleKey(cm, key, "mapping") && wasInsert && vim.insertMode) {
					if (key[0] == "<") {
						var lowerKey = key.toLowerCase().slice(1, -1);
						lowerKey = lowerKey.split("-").pop() || "";
						if (lowerKey == "lt") key = "<";
	...
	function vimKeyFromEvent(e, vim) {
		...
		if (key.length > 1 && key[0] == "n") key = key.replace("Numpad", "");
		key = specialKey[key] || key;
		var name = "";
		if (e.ctrlKey) name += "C-";
		if (e.altKey) name += "A-";
		if (e.metaKey) name += "M-";
		if (CodeMirror.isMac && e.altKey && !e.metaKey && !e.ctrlKey) name = name.slice(2);
	...
	defineOption("langmap", void 0, "string", ["lmap"], function(name, cm) {
		if (name === void 0) return langmap.string;
		else updateLangmap(name);
	});
	function InputState() {
		this.prefixRepeat = [];
		this.motionRepeat = [];
	...
	RegisterController.prototype = {
		pushText: function(registerName, operator, text, linewise, blockwise) {
			...
			if (registerName === "+" && typeof navigator !== "undefined" && typeof navigator.clipboard !== "undefined" && typeof navigator.clipboard.readText === "function") navigator.clipboard.writeText(text);
	...
	var commandDispatcher = {
		matchCommand: function(keys, keyMap, inputState, context) {
			...
			else if (!matches.full && matches.partial) return {
				type: "partial",
		...
		processSearch: function(cm, vim, command) {
			...
			function onPromptKeyUp(e, query$1, close) {
				...
				} else if (keyName && keyName != "<Left>" && keyName != "<Right>") vimGlobalState.searchHistoryController.reset();
				var parsedQuery;
				try {
			...
			function onPromptKeyDown(e, query$1, close) {
				...
				if (keyName == "<Esc>" || keyName == "<C-c>" || keyName == "<C-[>" || keyName == "<BS>" && query$1 == "") {
					vimGlobalState.searchHistoryController.pushInput(query$1);
					vimGlobalState.searchHistoryController.reset();
					...
					close();
					cm.focus();
				} else if (keyName == "<Up>" || keyName == "<Down>") CodeMirror.e_stop(e);
				else if (keyName == "<C-u>") {
					CodeMirror.e_stop(e);
					close("");
				}
		...
		processEx: function(cm, vim, command) {
			...
			function onPromptKeyDown(e, input, close) {
				...
				if (keyName == "<Up>" || keyName == "<Down>") {
					...
					CodeMirror.e_stop(e);
					close("");
				} else if (keyName && keyName != "<Left>" && keyName != "<Right>") vimGlobalState.exCommandHistoryController.reset();
			}
		...
		evalInput: function(cm, vim) {
			...
			if (motion) {
				...
				if (motionArgs.toJumplist) {
					if (!operator && cm.ace.curOp != null) cm.ace.curOp.command.scrollIntoView = "center-animate";
				...
				if (vim.visualMode) {
					if (!(vim.visualBlock && newHead.ch === Infinity)) newHead = clipCursorToContent(cm, newHead, oldHead);
					...
					if (cm.ace.curOp) cm.ace.curOp.vimDialogScroll = "center-animate";
	...
	var motions = {
		...
		moveByLines: function(cm, head, motionArgs, vim) {
			...
			if (fold) if (motionArgs.forward) {
				if (line > fold.start.row) line = fold.end.row + 1;
			} else line = fold.start.row;
		...
		moveToMatchedSymbol: function(cm, head) {
			...
			if (ch < lineText.length) {
				...
				return cm.findMatchingBracket(new Pos(line, ch + 1), { bracketRegex: re }).to;
			} else return cursor;
		},
		...
		textObjectManipulation: function(cm, head, motionArgs, vim) {
			...
			if (mirroredPairs[character]) {
				...
				if (!tmp) {
					...
					if (sc.find()) tmp = selectCompanionObject(cm, sc.from(), character, inclusive);
				}
				...
				if (head.ch > 0 && isEndOfSentenceSymbol(content[head.ch])) head.ch -= 1;
				var end = getSentence(cm, head, motionArgs.repeat, 1, inclusive);
				var start = getSentence(cm, head, motionArgs.repeat, -1, inclusive);
				if (isWhiteSpaceString(cm.getLine(start.line)[start.ch]) && isWhiteSpaceString(cm.getLine(end.line)[end.ch - 1])) start = {
					line: start.line,
					ch: start.ch + 1
				};
				tmp = {
			...
			else return expandSelection(cm, tmp.start, tmp.end, move);
		},
		...
		change: function(cm, args, ranges) {
			...
			if (!vim.visualMode) {
				...
				if (args.linewise) {
					...
					if (head.line > anchor.line) head = new Pos(head.line - 1, Number.MAX_VALUE);
				}
		...
		indent: function(cm, args, ranges) {
			...
			if (vim.visualBlock) {
				...
				for (var i = ranges.length - 1; i >= 0; i--) {
					...
					if (args.indentRight) cm.replaceRange(indent.repeat(repeat), cursor, cursor);
					else {
				...
				return cursor;
			} else if (cm.indentMore) for (var j = 0; j < repeat; j++) if (args.indentRight) cm.indentMore();
			else cm.indentLess();
			else {
	...
	var actions = {
		...
		scrollToCursor: function(cm, actionArgs) {
			...
			switch (actionArgs.position) {
				...
				case "bottom":
					...
					var lineHeight = cm.charCoords(lineLastCharPos, "local").bottom - y;
		...
		joinLines: function(cm, actionArgs, vim) {
			...
			for (var i = curStart.line; i < curEnd.line; i++) {
				...
				if (!actionArgs.keepSpaces) {
					...
					if (nextStartCh == -1) nextStartCh = nextLine.length;
					else text = " ";
				}
		...
		paste: function(cm, actionArgs, vim) {
			...
			if (actionArgs.registerName === "+" && typeof navigator !== "undefined" && typeof navigator.clipboard !== "undefined" && typeof navigator.clipboard.readText === "function") navigator.clipboard.readText().then((value) => {
		...
		continuePaste: function(cm, actionArgs, vim, text, register) {
			...
			if (vim.visualMode) {
				...
				if (linewise) {
					...
					if (!/\n/.test(text)) curPosFinal.ch += text.length - (actionArgs.after ? 1 : 0);
				}
			}
			if (vim.visualMode) exitVisualMode(cm, false);
		...
		oneNormalCommand: function(cm, actionArgs, vim) {
			...
			CodeMirror.on(cm, "vim-command-done", function handler() {
				...
				if (vim.insertModeReturn) {
					...
					if (!vim.insertMode) actions.enterInsertMode(cm, {}, vim);
				}
	...
	function clipCursorToContent(cm, cur, oldCur) {
		...
		if (56320 <= charCode && charCode <= 57343) {
			var direction = 1;
			if (oldCur && oldCur.line == line && oldCur.ch > ch) direction = -1;
			ch += direction;
			if (ch > maxCh) ch -= 2;
		}
	...
	function commandMatches(keys, keyMap, context, inputState) {
		...
		for (var i = noremap ? keyMap.length - defaultKeymapLength : 0; i < keyMap.length; i++) {
	...
	function expandWordUnderCursor(cm, { inclusive, innerWord, bigWord, noSymbol, multiline }, cursor) {
		...
		if (innerWord && /\s/.test(line.charAt(idx))) test = function(ch) {
			return /\s/.test(ch);
		};
		else {
		...
		} else while (test(line.charAt(end)) && end < line.length) end++;
		if (inclusive) {
			var wordEnd = end;
			var startsWithSpace = cur.ch <= start && /\s/.test(line.charAt(cur.ch));
			if (!startsWithSpace) while (/\s/.test(endLine.charAt(end)) && end < endLine.length) end++;
	...
	function expandTagUnderCursor(cm, head, inclusive) {
		var cur = head;
		if (!CodeMirror.findMatchingTag || !CodeMirror.findEnclosingTag) return {
			start: cur,
			end: cur
		};
	...
	function updateMark(cm, vim, markName, pos) {
		if (!inArray(markName, validMarks) && !latinCharRegex.test(markName)) return;
	...
	function findParagraph(cm, head, repeat, dir, inclusive) {
		...
		function isEmpty(i$1) {
			return !/\S/.test(cm.getLine(i$1));
		}
		function isBoundary(i$1, dir$1, any) {
			if (any) return isEmpty(i$1) != isEmpty(i$1 + dir$1);
			return !isEmpty(i$1) && isEmpty(i$1 + dir$1);
		}
		function skipFold(i$1) {
			dir = dir > 0 ? 1 : -1;
			var foldLine = cm.ace.session.getFoldLine(i$1);
			if (foldLine) {
				if (i$1 + dir > foldLine.start.row && i$1 + dir < foldLine.end.row) dir = (dir > 0 ? foldLine.end.row : foldLine.start.row) - i$1;
			}
		}
		if (dir) {
	...
	function getSentence(cm, cur, repeat, dir, inclusive) {
		function nextChar(curr) {
			if (curr.pos + curr.dir < 0 || curr.pos + curr.dir >= curr.line.length) curr.line = null;
			else curr.pos += curr.dir;
		}
		function forward(cm$1, ln, pos, dir$1) {
			var curr = {
				line: cm$1.getLine(ln),
				ln,
				pos,
				dir: dir$1
			};
			...
			while (curr.line !== null) {
				...
				if (isEndOfSentenceSymbol(curr.line[curr.pos])) if (!inclusive) return {
					ln: curr.ln,
					pos: curr.pos + 1
				};
				else {
					nextChar(curr);
					while (curr.line !== null) if (isWhiteSpaceString(curr.line[curr.pos])) {
						lastSentencePos = curr.pos;
						nextChar(curr);
					} else break;
					return {
		...
		function reverse(cm$1, ln, pos, dir$1) {
			...
			nextChar(curr);
			while (curr.line !== null) {
				if (!isWhiteSpaceString(curr.line[curr.pos]) && !isEndOfSentenceSymbol(curr.line[curr.pos])) lastSentencePos = curr.pos;
				else if (isEndOfSentenceSymbol(curr.line[curr.pos])) if (!inclusive) return {
					ln: curr.ln,
					pos: lastSentencePos
				};
				else if (isWhiteSpaceString(curr.line[curr.pos + 1])) return {
					ln: curr.ln,
					pos: curr.pos + 1
				};
				else return {
			...
			curr.line = line;
			if (inclusive && isWhiteSpaceString(curr.line[curr.pos])) return {
				ln: curr.ln,
				pos: curr.pos
			};
			else return {
	...
	function showPrompt(cm, options$1) {
		if (keyToKeyStack.length) {
			if (!options$1.value) options$1.value = "";
			virtualPrompt = options$1;
			return;
		}
	...
	function highlightSearchMatches(cm, query) {
		...
		highlightTimeout = setTimeout(function() {
			...
			var searchState$1 = getSearchState(cm);
			searchState$1.highlightTimeout = null;
			var overlay = searchState$1.getOverlay();
			if (!overlay || query != overlay.query) {
				if (overlay) cm.removeOverlay(overlay);
				overlay = searchOverlay(query);
				cm.addOverlay(overlay);
				if (cm.showMatchesOnScrollbar) {
					if (searchState$1.getScrollbarAnnotate()) searchState$1.getScrollbarAnnotate().clear();
					searchState$1.setScrollbarAnnotate(cm.showMatchesOnScrollbar(query));
				}
				searchState$1.setOverlay(overlay);
			}
		}, 50);
	}
	...
	function getLastEditPos(cm) {
		if (cm.getLastEditEnd) return cm.getLastEditEnd();
	...
	ExCommandDispatcher.prototype = {
		...
		parseInput_: function(cm, inputStream, result) {
			...
			if (result.line == void 0) if (cm.state.vim.visualMode) {
			...
			} else result.selectionLine = cm.getCursor().line;
			else {
		...
		map: function(lhs, rhs, ctx, noremap$1) {
			if (lhs != ":" && lhs.charAt(0) == ":") {
	...
	var exCommands = {
		...
		set: function(cm, params) {
			...
			if (optionIsBoolean) {
				if (forceToggle) value = !getOption(optionName, cm, setCfg);
		...
		normal: function(cm, params) {
			...
			if (typeof line == "number") {
				...
				for (var i = line; i <= lineEnd; i++) {
					...
					if (cm.state.vim.insertMode) exitInsertMode(cm, true);
				}
			} else {
				doKeyToKey(cm, params.argString.trimStart());
				if (cm.state.vim.insertMode) exitInsertMode(cm, true);
		...
		global: function(cm, params) {
			...
			for (var i = lineStart; i <= lineEnd; i++) {
				...
				if (query.test(line) !== inverted) matchedLines.push(cmd ? cm.getLineHandle(i) : line);
			}
			if (!cmd) {
			...
			var nextCommand = function() {
				...
				} else if (cm.releaseLineHandles) cm.releaseLineHandles();
			};
			nextCommand();
		},
		...
		write: function(cm) {
			if (CodeMirror.commands.save) CodeMirror.commands.save(cm);
	...
	function handleExternalSelection(cm, vim, keepHPos) {
		...
		} else if (!vim.insertMode && !keepHPos) vim.lastHPos = cm.getCursor().ch;
	}
	...
	function repeatInsertModeChanges(cm, changes, repeat) {
		...
		for (var i = 0; i < repeat; i++) {
			...
			for (var j = 0; j < changes.length; j++) {
				...
				if (change instanceof InsertModeKey) sendCmKey(cm, change.keyName, change);
				else if (typeof change == "string") cm.replaceSelection(change);
				else {
	...
	function multiSelectHandleKey(cm, key, origin) {
		...
		if (vim.wasInVisualBlock && !wasMultiselect) vim.wasInVisualBlock = false;
		else if (wasMultiselect && vim.visualBlock) vim.wasInVisualBlock = true;
		if (key == "<Esc>" && !vim.insertMode && !vim.visualMode && wasMultiselect) cm.ace.exitMultiSelectMode();
		else if (visualBlock || !wasMultiselect || cm.ace.inVirtualSelectionMode) isHandled = vimApi.handleKey(cm, key, origin);
		else {
			...
			cm.operation(function() {
				...
				cm.ace.forEachSelection(function() {
					...
					if (cm.ace.inVirtualSelectionMode) changeQueueList[index] = cm.state.vim.inputState.changeQueue;
					if (cm.virtualSelectionMode()) cm.state.vim = cloneVimState(old);
					index++;
				});
		...
		if (isHandled && !vim.visualMode && !vim.insert && vim.visualMode != cm.somethingSelected()) handleExternalSelection(cm, vim, true);
		return isHandled;
	}
	resetVimGlobalState();
	...
	exports.handler = {
		...
		drawCursor: function(element, pixelPos, config, sel, session) {
			...
			if (!vim.insertMode) {
				if (!(!sel.cursor ? session.selection.isBackwards() || session.selection.isEmpty() : Range.comparePoints(sel.cursor, sel.start) <= 0) && left > w) left -= w;
			}
		...
		$getDirectionForHighlight: function(editor) {
			...
			if (!getVim(cm).insertMode) return editor.session.selection.isBackwards() || editor.session.selection.isEmpty();
		},
		handleKeyboard: function(data, hashId, key, keyCode, e) {
			...
			if (!vim.insertMode) if (hashId == -1) {
				...
				data.inputChar = key;
			} else if (hashId == 4 || hashId == 0) if (data.inputKey == key && data.inputHash == hashId && data.inputChar) {
				key = data.inputChar;
				hashId = -1;
			} else {
				data.inputChar = null;
			...
			else data.inputChar = data.inputKey = null;
			if (cm.state.overwrite && vim.insertMode && key == "backspace" && hashId == 0) return { command: "gotoleft" };
			if (key == "c" && hashId == 1) {
	...
	defineOption("textwidth", void 0, "number", ["tw"], function(width, cm) {
		if (cm === void 0) return;
		if (width === void 0) return cm.ace.getOption("printMarginColumn");
		else {
			var column = Math.round(width);
			if (column > 1) cm.ace.setOption("printMarginColumn", column);
		}
	});
	...
	function delayedExecAceCommand(op, ace) {
		...
		if (cmd) ace.execCommand(cmd.exec ? cmd : cmd.name, cmd.args);

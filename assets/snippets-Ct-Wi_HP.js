...
import { i as require_clipboard, t as require_editor } from "./editor-BiOsjB7l.js";
import { n as require_range_list } from "./edit_session-CDHRvoey.js";
...
var require_snippets = /* @__PURE__ */ __commonJSMin(((exports) => {
	...
	var SnippetManager = class SnippetManager {
			...
			return str.replace(re, function() {
				...
				for (var i = 0; i < fmtParts.length; i++) {
					...
					if (typeof ch$1 == "object") {
						fmtParts[i] = "";
						if (ch$1.changeCase && ch$1.local) {
							var next = fmtParts[i + 1];
							if (next && typeof next == "string") {
								if (ch$1.changeCase == "u") fmtParts[i] = next[0].toUpperCase();
								else fmtParts[i] = next[0].toLowerCase();
								fmtParts[i + 1] = next.substr(1);
							}
						} else if (ch$1.changeCase) gChangeCase = ch$1.changeCase;
					} else if (gChangeCase == "U") fmtParts[i] = ch$1.toUpperCase();
					else if (gChangeCase == "L") fmtParts[i] = ch$1.toLowerCase();
				}
		...
		resolveVariables(snippet, editor) {
			...
			function gotoNext(ch$1) {
				var i1 = snippet.indexOf(ch$1, i + 1);
				if (i1 != -1) i = i1;
			}
			return result;
		}
	...
	var processSnippetText = function(editor, snippetText, options = {}) {
		...
		var tabstops = [];
		tokens.forEach(function(p$1, i$1) {
			if (typeof p$1 != "object") return;
			var id$1 = p$1.tabstopId;
			var ts$1 = tabstops[id$1];
			if (!ts$1) {
				ts$1 = tabstops[id$1] = [];
				ts$1.index = id$1;
				ts$1.value = "";
			...
			if (ts$1.indexOf(p$1) !== -1) return;
			if (p$1.choices && !ts$1.choices) ts$1.choices = p$1.choices;
			ts$1.push(p$1);
			var i1$1 = tokens.indexOf(p$1, i$1 + 1);
			if (i1$1 === -1) return;
			var value$1 = tokens.slice(i$1 + 1, i1$1);
			if (value$1.some(function(t) {
				return typeof t === "object";
			}) && !ts$1.value) ts$1.value = value$1;
			else if (value$1.length && (!ts$1.value || typeof ts$1.value !== "string")) ts$1.value = value$1.join("");
		});
		tabstops.forEach(function(ts$1) {
			ts$1.length = 0;
		});
		var expanding = {};
		function copyValue(val) {
			...
			for (var i$1 = 0; i$1 < val.length; i$1++) {
				...
				if (typeof p$1 == "object") {
					if (expanding[p$1.tabstopId]) continue;
					p$1 = copy[val.lastIndexOf(p$1, i$1 - 1)] || { tabstopId: p$1.tabstopId };
				}
				copy[i$1] = p$1;
			}
			return copy;
	...
	var TabstopManager = class {
		...
		addTabstops(tabstops, start, end) {
			...
			tabstops.forEach(function(ts, index) {
				...
				for (var i = 0; i < ts.length; i++) {
					var p$1 = ts[i];
					var range = Range.fromPoints(p$1.start, p$1.end || p$1.start);
					...
					if (p$1.fmtString || dest.firstNonLinked && useLink) {
						range.linked = true;

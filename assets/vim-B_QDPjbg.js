...
import { r as require_search } from "./editor-ImsyhaOB.js";
import { i as require_search_highlight } from "./edit_session-DAgqly_m.js";
import "./tooltip-BuOUCQpw.js";
import "./tokenizer-B5s1nUwH.js";
import { t as require_text } from "./text-D8sm5DzM.js";
import "./token_iterator-BNxpI84f.js";
import "./hash_handler-DWXab_Mk.js";
import { n as require_multi_select_commands, t as require_multi_select } from "./multi_select-Dohhryzh.js";
import { t as require_hardwrap } from "./hardwrap-Dmpwch-c.js";
//#region node_modules/ace-code/src/keyboard/vim.js
/**
	...
	(function() {
		...
		this.scanForBracket = function(pos, dir, _, options) {
			...
			if (dir == 1) var m = this.ace.session.$findClosingBracket(re.slice(1, 2), toAcePos(pos), tokenRe);
				...
				if (!m && options.bracketRegex && options.bracketRegex.test(this.getLine(pos.line)[pos.ch - 1])) m = {
					row: pos.line,
					column: pos.ch - 1
				};
			}
...
.ace_dialog input {
  ...
  font-family: monospace;
	...
	*/
	...
	var upperCaseChars;
	try {
		upperCaseChars = /* @__PURE__ */ new RegExp("^[\\p{Lu}]$", "u");
	} catch (_) {
		upperCaseChars = /^[A-Z]$/;
	}
	...
	function sendKeyToPrompt(key) {
		if (key[0] == "<") {
			...
			if (lowerKey == "lt") key = "<";
				...
				if (virtualPrompt.onKeyDown) virtualPrompt.onKeyDown(event, virtualPrompt.value, close);
				if (virtualPrompt && virtualPrompt.onKeyUp) virtualPrompt.onKeyUp(event, virtualPrompt.value, close);
				return;
			}
		}
		if (key == "\n") {
	...
	*/
	...
	function showPrompt(cm, options) {
		...
		if (cm.openDialog) cm.openDialog(template, options.onClose, {
	...
	*/
	...
	ExCommandDispatcher.prototype = {
		...
		parseInput_: function(cm, inputStream, result) {
			...
			if (commandMatch) result.commandName = commandMatch[1];
	...
	var exCommands = {
		...
		sort: function(cm, params) {
			...
			if (number || pattern) for (var i = 0; i < text.length; i++) {
				...
				else if (!pattern && numberRegex.exec(text[i])) numPart.push(text[i]);

...
var require_whitespace = /* @__PURE__ */ __commonJSMin(((exports) => {
	...
	exports.$detectIndentation = function(lines, fallback) {
		...
		function getScore(indent) {
			var score$1 = 0;
			for (var i$1 = indent; i$1 < stats.length; i$1 += indent) score$1 += stats[i$1] || 0;
			return score$1;
		}
		...
		if (tabIndents > spaceIndents + 1) {
			if (tabLength == 1 || spaceIndents < tabIndents / 4 || first.score < 1.8) tabLength = void 0;
			return {
				ch: "	",
				length: tabLength
			};
	...
	exports.trimTrailingSpace = function(session, options) {
		...
		if (options && options.keepCursorPosition) {
			if (session.selection.rangeCount) session.selection.rangeList.ranges.forEach(function(x, i$1, ranges) {
				var next = ranges[i$1 + 1];
				if (next && next.cursor.row == x.cursor.row) return;
				cursors.push(x.cursor);
			});
			else cursors.push(session.selection.getCursor());
			ci = 0;
		}

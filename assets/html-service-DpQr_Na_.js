import { C as SymbolKind, D as init_main, O as __commonJSMin, S as SymbolInformation, T as TextEdit, b as Range, g as InsertTextFormat, h as FoldingRangeKind, l as CompletionItemKind, m as DocumentSymbol, n as BaseService, p as DocumentHighlightKind, t as CommonConverter, v as MarkupKind, x as SelectionRange, y as Position } from "./common-converters-BtzeF4y0.js";
import { n as mergeObjects, r as URI, t as checkValueAgainstRegexpArray } from "./webworker-CFwJXhOX.js";
	...
	var HTMLParser$1 = class {
		...
		parse(html) {
			...
			while (match = regTag.exec(html)) {
				...
				if (matchIndex > lastIndex) {
					...
					else saveBlock("text", text, lastIndex);
				}
				...
				if (tagCDATA) arrCDATA.push(match[0]);
					...
					} else saveBlock("text", match[0], matchIndex);
		...
		addListener(types, listener) {
			...
			for (let i = 0, l = arrTypes.length; i < l; i++) {
				type = arrTypes[i];
				if (_listeners[type] === void 0) _listeners[type] = [];
		...
		fire(type, data) {
			...
			if (listenersType !== void 0) listeners = listeners.concat(listenersType);
			if (listenersAll !== void 0) listeners = listeners.concat(listenersAll);
			...
			this.lastEvent = data;
			for (let i = 0, l = listeners.length; i < l; i++) listeners[i].call(this, data);
		}
		removeListener(type, listener) {
			const listenersType = this._listeners[type];
			if (listenersType !== void 0) {
				for (let i = 0, l = listenersType.length; i < l; i++) if (listenersType[i] === listener) {
		...
		fixPos(event, index) {
			const arrLines = event.raw.substr(0, index).split(/\r?\n/);
...
var require_alt_require = /* @__PURE__ */ __commonJSMin(((exports) => {
	Object.defineProperty(exports, "__esModule", { value: true });
	exports.default = {
		id: "alt-require",
		...
		init(parser, reporter) {
			parser.addListener("tagstart", (event) => {
				...
				let selector;
				if (tagName === "img" && !("alt" in mapAttrs)) reporter.warn("An alt attribute must be present on <img> elements.", event.line, col, this, event.raw);
...
var require_attr_lowercase = /* @__PURE__ */ __commonJSMin(((exports) => {
	Object.defineProperty(exports, "__esModule", { value: true });
	const svgIgnores = [
		"allowReorder",
		"attributeName",
	...
	function testAgainstStringOrRegExp(value, comparison) {
		if (comparison instanceof RegExp) return comparison.test(value) ? {
			match: value,
			pattern: comparison
		} : false;
		...
		if (comparisonIsRegex) return hasCaseInsensitiveFlag ? new RegExp(comparison.slice(1, -2), "i").test(value) : new RegExp(comparison.slice(1, -1)).test(value);
		...
		init(parser, reporter, options) {
			...
			parser.addListener("tagstart", (event) => {
				...
				for (let i = 0, l = attrs.length; i < l; i++) {
					...
					if (!exceptions.find((exp) => testAgainstStringOrRegExp(attrName, exp)) && attrName !== attrName.toLowerCase()) reporter.error(`The attribute name of [ ${attrName} ] must be in lowercase.`, event.line, col + attr.index, this, attr.raw);
				}
			});
		}
	};
}));
var require_attr_sorted = /* @__PURE__ */ __commonJSMin(((exports) => {
	Object.defineProperty(exports, "__esModule", { value: true });
	exports.default = {
		id: "attr-sorted",
		...
		init(parser, reporter) {
			...
			for (let i = 0; i < sortOrder.length; i++) orderMap[sortOrder[i]] = i;
			parser.addListener("tagstart", (event) => {
				const attrs = event.attrs;
				const listOfAttributes = [];
				for (let i = 0; i < attrs.length; i++) listOfAttributes.push(attrs[i].name);
				const originalAttrs = JSON.stringify(listOfAttributes);
				listOfAttributes.sort((a, b) => {
					if (orderMap[a] == void 0 && orderMap[b] == void 0) return 0;
					if (orderMap[a] == void 0) return 1;
					else if (orderMap[b] == void 0) return -1;
					return orderMap[a] - orderMap[b] || a.localeCompare(b);
				});
				if (originalAttrs !== JSON.stringify(listOfAttributes)) reporter.error(`Inaccurate order ${originalAttrs} should be in hierarchy ${JSON.stringify(listOfAttributes)} `, event.line, event.col, this, event.raw);
			});
		}
	};
}));
var require_attr_no_duplication = /* @__PURE__ */ __commonJSMin(((exports) => {
	Object.defineProperty(exports, "__esModule", { value: true });
	exports.default = {
		...
		init(parser, reporter) {
			parser.addListener("tagstart", (event) => {
				...
				for (let i = 0, l = attrs.length; i < l; i++) {
					attr = attrs[i];
					attrName = attr.name;
					if (mapAttrName[attrName] === true) reporter.error(`Duplicate of attribute name [ ${attr.name} ] was found.`, event.line, col + attr.index, this, attr.raw);
...
var require_attr_unsafe_chars = /* @__PURE__ */ __commonJSMin(((exports) => {
	Object.defineProperty(exports, "__esModule", { value: true });
	exports.default = {
...
var require_attr_value_double_quotes = /* @__PURE__ */ __commonJSMin(((exports) => {
	Object.defineProperty(exports, "__esModule", { value: true });
	exports.default = {
		...
		init(parser, reporter) {
			parser.addListener("tagstart", (event) => {
				...
				for (let i = 0, l = attrs.length; i < l; i++) {
					attr = attrs[i];
					if (attr.value !== "" && attr.quote !== "\"" || attr.value === "" && attr.quote === "'") reporter.error(`The value of attribute [ ${attr.name} ] must be in double quotes.`, event.line, col + attr.index, this, attr.raw);
				}
			});
		}
	};
}));
var require_attr_value_not_empty = /* @__PURE__ */ __commonJSMin(((exports) => {
	Object.defineProperty(exports, "__esModule", { value: true });
	exports.default = {
		...
		init(parser, reporter) {
			parser.addListener("tagstart", (event) => {
				...
				for (let i = 0, l = attrs.length; i < l; i++) {
					attr = attrs[i];
					if (attr.quote === "" && attr.value === "") reporter.warn(`The attribute [ ${attr.name} ] must have a value.`, event.line, col + attr.index, this, attr.raw);
				}
			});
		}
	};
}));
var require_attr_value_single_quotes = /* @__PURE__ */ __commonJSMin(((exports) => {
	Object.defineProperty(exports, "__esModule", { value: true });
	exports.default = {
		...
		init(parser, reporter) {
			parser.addListener("tagstart", (event) => {
				...
				for (let i = 0, l = attrs.length; i < l; i++) {
					attr = attrs[i];
					if (attr.value !== "" && attr.quote !== "'" || attr.value === "" && attr.quote === "\"") reporter.error(`The value of attribute [ ${attr.name} ] must be in single quotes.`, event.line, col + attr.index, this, attr.raw);
				}
			});
		}
	};
}));
var require_attr_whitespace = /* @__PURE__ */ __commonJSMin(((exports) => {
	Object.defineProperty(exports, "__esModule", { value: true });
	exports.default = {
		id: "attr-whitespace",
		...
		init(parser, reporter, options) {
			...
			parser.addListener("tagstart", (event) => {
				...
				attrs.forEach((elem) => {
					...
					if (exceptions.indexOf(attrName) !== -1) return;
					if (elem.value.trim() !== elem.value) reporter.error(`The attributes of [ ${attrName} ] must not have leading or trailing whitespace.`, event.line, col + attr.index, this, attr.raw);
					if (elem.value.replace(/ +(?= )/g, "") !== elem.value) reporter.error(`The attributes of [ ${attrName} ] must be separated by only one space.`, event.line, col + attr.index, this, attr.raw);
				});
			});
		}
	};
}));
var require_doctype_first = /* @__PURE__ */ __commonJSMin(((exports) => {
	Object.defineProperty(exports, "__esModule", { value: true });
	exports.default = {
		id: "doctype-first",
		...
		init(parser, reporter) {
			const allEvent = (event) => {
				if (event.type === "start" || event.type === "text" && /^\s*$/.test(event.raw)) return;
				if (event.type !== "comment" && event.long === false || /^DOCTYPE\s+/i.test(event.content) === false) reporter.error("Doctype must be declared first.", event.line, event.col, this, event.raw);
...
var require_doctype_html5 = /* @__PURE__ */ __commonJSMin(((exports) => {
	Object.defineProperty(exports, "__esModule", { value: true });
	exports.default = {
		id: "doctype-html5",
		...
		init(parser, reporter) {
			const onComment = (event) => {
				if (event.long === false && event.content.toLowerCase() !== "doctype html") reporter.warn("Invalid doctype. Use: \"<!DOCTYPE html>\"", event.line, event.col, this, event.raw);
			};
			const onTagStart = () => {
...
var require_head_script_disabled = /* @__PURE__ */ __commonJSMin(((exports) => {
	Object.defineProperty(exports, "__esModule", { value: true });
	exports.default = {
		...
		init(parser, reporter) {
			...
			const onTagStart = (event) => {
				const type = parser.getMapAttrs(event.attrs).type;
				const tagName = event.tagName.toLowerCase();
				if (tagName === "head") isInHead = true;
				if (isInHead === true && tagName === "script" && (!type || reScript.test(type) === true)) reporter.warn("The <script> tag cannot be used in a <head> tag.", event.line, event.col, this, event.raw);
			};
...
var require_href_abs_or_rel = /* @__PURE__ */ __commonJSMin(((exports) => {
	Object.defineProperty(exports, "__esModule", { value: true });
	exports.default = {
		id: "href-abs-or-rel",
		...
		init(parser, reporter, options) {
			...
			parser.addListener("tagstart", (event) => {
				...
				for (let i = 0, l = attrs.length; i < l; i++) {
					attr = attrs[i];
					if (attr.name === "href") {
						if (hrefMode === "absolute" && /^\w+?:/.test(attr.value) === false || hrefMode === "relative" && /^https?:\/\//.test(attr.value) === true) reporter.warn(`The value of the href attribute [ ${attr.value} ] must be ${hrefMode}.`, event.line, col + attr.index, this, attr.raw);
						break;
					}
				}
			});
...
var require_html_lang_require = /* @__PURE__ */ __commonJSMin(((exports) => {
	Object.defineProperty(exports, "__esModule", { value: true });
	const languageTag = `((?<grandfathered>(en-GB-oed|i-ami|i-bnn|i-default|i-enochian|i-hak|i-klingon|i-lux|i-mingo|i-navajo|i-pwn|i-tao|i-tay|i-tsu|sgn-BE-FR|sgn-BE-NL|sgn-CH-DE)|(art-lojban|cel-gaulish|no-bok|no-nyn|zh-guoyu|zh-hakka|zh-min|zh-min-nan|zh-xiang))|((?<language>([A-Za-z]{2,3}(-(?<extlang>[A-Za-z]{3}(-[A-Za-z]{3}){0,2}))?)|[A-Za-z]{4}|[A-Za-z]{5,8})(-(?<script>[A-Za-z]{4}))?(-(?<region>[A-Za-z]{2}|[0-9]{3}))?(-(?<variant>[A-Za-z0-9]{5,8}|[0-9][A-Za-z0-9]{3}))*(-(?<extension>[0-9A-WY-Za-wy-z](-[A-Za-z0-9]{2,8})+))*(-(?<privateUse>x(-[A-Za-z0-9]{1,8})+))?)|(?<privateUse2>x(-[A-Za-z0-9]{1,8})+))`;
	exports.default = {
		...
		init(parser, reporter) {
			parser.addListener("tagstart", (event) => {
				...
				if (tagName === "html") if ("lang" in mapAttrs) {
					if (!mapAttrs["lang"]) reporter.warn("The lang attribute of <html> element must have a value.", event.line, col, this, event.raw);
					else if (!langValidityPattern.test(mapAttrs["lang"])) reporter.warn("The lang attribute value of <html> element must be a valid BCP47.", event.line, col, this, event.raw);
				} else reporter.warn("An lang attribute must be present on <html> elements.", event.line, col, this, event.raw);
			});
		}
	};
}));
var require_id_class_ad_disabled = /* @__PURE__ */ __commonJSMin(((exports) => {
	Object.defineProperty(exports, "__esModule", { value: true });
	exports.default = {
			...
			parser.addListener("tagstart", (event) => {
				...
				for (let i = 0, l = attrs.length; i < l; i++) {
					...
					if (/^(id|class)$/i.test(attrName)) {
						if (/(^|[-_])ad([-_]|$)/i.test(attr.value)) reporter.warn(`The value of attribute ${attrName} cannot use the ad keyword.`, event.line, col + attr.index, this, attr.raw);
					}
				}
			});
		}
...
var require_id_class_value = /* @__PURE__ */ __commonJSMin(((exports) => {
	Object.defineProperty(exports, "__esModule", { value: true });
	exports.default = {
		id: "id-class-value",
		...
		init(parser, reporter, options) {
			...
			let rule;
			if (typeof options === "string") rule = arrRules[options];
			else rule = options;
			if (typeof rule === "object" && rule.regId) {
				let regId = rule.regId;
				const message = rule.message;
				if (!(regId instanceof RegExp)) regId = new RegExp(regId);
				parser.addListener("tagstart", (event) => {
					const attrs = event.attrs;
					let attr;
					const col = event.col + event.tagName.length + 1;
					for (let i = 0, l1 = attrs.length; i < l1; i++) {
						...
						if (attr.name.toLowerCase() === "id") {
							if (regId.test(attr.value) === false) reporter.warn(message, event.line, col + attr.index, this, attr.raw);
						}
						if (attr.name.toLowerCase() === "class") {
							...
							for (let j = 0, l2 = arrClass.length; j < l2; j++) {
								...
								if (classValue && regId.test(classValue) === false) reporter.warn(message, event.line, col + attr.index, this, classValue);
							}
						}
					}
				});
...
var require_id_unique = /* @__PURE__ */ __commonJSMin(((exports) => {
	Object.defineProperty(exports, "__esModule", { value: true });
	exports.default = {
		id: "id-unique",
			...
			parser.addListener("tagstart", (event) => {
				...
				for (let i = 0, l = attrs.length; i < l; i++) {
					...
					if (attr.name.toLowerCase() === "id") {
						id = attr.value;
						if (id) {
							if (mapIdCount[id] === void 0) mapIdCount[id] = 1;
							else mapIdCount[id]++;
							if (mapIdCount[id] > 1) reporter.error(`The id value [ ${id} ] must be unique.`, event.line, col + attr.index, this, attr.raw);
						}
						break;
					}
				}
...
var require_inline_script_disabled = /* @__PURE__ */ __commonJSMin(((exports) => {
	Object.defineProperty(exports, "__esModule", { value: true });
	exports.default = {
		...
		init(parser, reporter) {
			parser.addListener("tagstart", (event) => {
				...
				for (let i = 0, l = attrs.length; i < l; i++) {
					...
					if (reEvent.test(attrName) === true) reporter.warn(`Inline script [ ${attr.raw} ] cannot be used.`, event.line, col + attr.index, this, attr.raw);
					else if (attrName === "src" || attrName === "href") {
						if (/^\s*javascript:/i.test(attr.value)) reporter.warn(`Inline script [ ${attr.raw} ] cannot be used.`, event.line, col + attr.index, this, attr.raw);
					}
				}
			});
		}
...
var require_inline_style_disabled = /* @__PURE__ */ __commonJSMin(((exports) => {
	Object.defineProperty(exports, "__esModule", { value: true });
	exports.default = {
		...
		init(parser, reporter) {
			parser.addListener("tagstart", (event) => {
				...
				for (let i = 0, l = attrs.length; i < l; i++) {
					attr = attrs[i];
					if (attr.name.toLowerCase() === "style") reporter.warn(`Inline style [ ${attr.raw} ] cannot be used.`, event.line, col + attr.index, this, attr.raw);
				}
			});
		}
	};
}));
var require_input_requires_label = /* @__PURE__ */ __commonJSMin(((exports) => {
	Object.defineProperty(exports, "__esModule", { value: true });
	exports.default = {
		...
		init(parser, reporter) {
			...
			parser.addListener("tagstart", (event) => {
				...
				const col = event.col + tagName.length + 1;
				if (tagName === "input") {
					if (mapAttrs["type"] !== "hidden") inputTags.push({
						event,
						col,
						id: mapAttrs["id"]
					});
				...
				if (tagName === "label") {
					if ("for" in mapAttrs && mapAttrs["for"] !== "") labelTags.push({
						event,
						col,
			...
			parser.addListener("end", () => {
				inputTags.forEach((inputTag) => {
					if (!hasMatchingLabelTag(inputTag)) reporter.warn("No matching [ label ] tag found.", inputTag.event.line, inputTag.col, this, inputTag.event.raw);
				});
			});
			function hasMatchingLabelTag(inputTag) {
				let found = false;
				labelTags.forEach((labelTag) => {
					if (inputTag.id && inputTag.id === labelTag.forValue) found = true;
				});
				return found;
			}
		}
...
var require_script_disabled = /* @__PURE__ */ __commonJSMin(((exports) => {
	Object.defineProperty(exports, "__esModule", { value: true });
	exports.default = {
		id: "script-disabled",
		...
		init(parser, reporter) {
			parser.addListener("tagstart", (event) => {
				if (event.tagName.toLowerCase() === "script") reporter.error("The <script> tag cannot be used.", event.line, event.col, this, event.raw);
			});
		}
	};
}));
var require_space_tab_mixed_disabled = /* @__PURE__ */ __commonJSMin(((exports) => {
	Object.defineProperty(exports, "__esModule", { value: true });
	exports.default = {
		...
		init(parser, reporter, options) {
			...
			parser.addListener("text", (event) => {
				...
				while (match = reMixed.exec(raw)) {
					...
					if (fixedPos.col !== 1) continue;
					const whiteSpace = match[2];
					if (indentMode === "space") {
						if (spaceLengthRequire) {
							if (/^ +$/.test(whiteSpace) === false || whiteSpace.length % spaceLengthRequire !== 0) reporter.warn(`Please use space for indentation and keep ${spaceLengthRequire} length.`, fixedPos.line, 1, this, event.raw);
						} else if (/^ +$/.test(whiteSpace) === false) reporter.warn("Please use space for indentation.", fixedPos.line, 1, this, event.raw);
					} else if (indentMode === "tab" && /^\t+$/.test(whiteSpace) === false) reporter.warn("Please use tab for indentation.", fixedPos.line, 1, this, event.raw);
					else if (/ +\t|\t+ /.test(whiteSpace) === true) reporter.warn("Do not mix tabs and spaces for indentation.", fixedPos.line, 1, this, event.raw);
				}
			});
		}
	};
}));
var require_spec_char_escape = /* @__PURE__ */ __commonJSMin(((exports) => {
	Object.defineProperty(exports, "__esModule", { value: true });
	exports.default = {
		id: "spec-char-escape",
...
var require_src_not_empty = /* @__PURE__ */ __commonJSMin(((exports) => {
	Object.defineProperty(exports, "__esModule", { value: true });
	exports.default = {
		id: "src-not-empty",
		...
		init(parser, reporter) {
			parser.addListener("tagstart", (event) => {
				...
				for (let i = 0, l = attrs.length; i < l; i++) {
					attr = attrs[i];
					if ((/^(img|script|embed|bgsound|iframe)$/.test(tagName) === true && attr.name === "src" || tagName === "link" && attr.name === "href" || tagName === "object" && attr.name === "data") && attr.value === "") reporter.error(`The attribute [ ${attr.name} ] of the tag [ ${tagName} ] must have a value.`, event.line, col + attr.index, this, attr.raw);
				}
			});
		}
	};
}));
var require_style_disabled = /* @__PURE__ */ __commonJSMin(((exports) => {
	Object.defineProperty(exports, "__esModule", { value: true });
	exports.default = {
		id: "style-disabled",
		...
		init(parser, reporter) {
			parser.addListener("tagstart", (event) => {
				if (event.tagName.toLowerCase() === "style") reporter.warn("The <style> tag cannot be used.", event.line, event.col, this, event.raw);
			});
		}
	};
}));
var require_tag_pair = /* @__PURE__ */ __commonJSMin(((exports) => {
	Object.defineProperty(exports, "__esModule", { value: true });
	exports.default = {
		id: "tag-pair",
		...
		init(parser, reporter) {
			...
			parser.addListener("tagstart", (event) => {
				const tagName = event.tagName.toLowerCase();
				if (mapEmptyTags[tagName] === void 0 && !event.close) stack.push({
					tagName,
					line: event.line,
					raw: event.raw
				});
			...
			parser.addListener("tagend", (event) => {
				const tagName = event.tagName.toLowerCase();
				let pos;
				for (pos = stack.length - 1; pos >= 0; pos--) if (stack[pos].tagName === tagName) break;
				if (pos >= 0) {
					const arrTags = [];
					for (let i = stack.length - 1; i > pos; i--) arrTags.push(`</${stack[i].tagName}>`);
					if (arrTags.length > 0) {
					...
					stack.length = pos;
				} else reporter.error(`Tag must be paired, no start tag: [ ${event.raw} ]`, event.line, event.col, this, event.raw);
			});
			parser.addListener("end", (event) => {
				const arrTags = [];
				for (let i = stack.length - 1; i >= 0; i--) arrTags.push(`</${stack[i].tagName}>`);
				if (arrTags.length > 0) {
					const lastEvent = stack[stack.length - 1];
...
var require_tag_self_close = /* @__PURE__ */ __commonJSMin(((exports) => {
	Object.defineProperty(exports, "__esModule", { value: true });
	exports.default = {
		id: "tag-self-close",
		...
		init(parser, reporter) {
			...
			parser.addListener("tagstart", (event) => {
				...
				if (mapEmptyTags[tagName] !== void 0) {
					if (!event.close) reporter.warn(`The empty tag : [ ${tagName} ] must be self closed.`, event.line, event.col, this, event.raw);
				}
			});
		}
	};
}));
var require_empty_tag_not_self_closed = /* @__PURE__ */ __commonJSMin(((exports) => {
	Object.defineProperty(exports, "__esModule", { value: true });
	exports.default = {
		...
		init(parser, reporter) {
			...
			parser.addListener("tagstart", (event) => {
				const tagName = event.tagName.toLowerCase();
				if (mapEmptyTags[tagName] !== void 0) {
					if (event.close) reporter.error(`The empty tag : [ ${tagName} ] must not use self closed syntax.`, event.line, event.col, this, event.raw);
				}
			});
		}
	};
}));
var require_tagname_lowercase = /* @__PURE__ */ __commonJSMin(((exports) => {
	Object.defineProperty(exports, "__esModule", { value: true });
	exports.default = {
		...
		init(parser, reporter, options) {
			...
			parser.addListener("tagstart,tagend", (event) => {
				const tagName = event.tagName;
				if (exceptions.indexOf(tagName) === -1 && tagName !== tagName.toLowerCase()) reporter.error(`The html element name of [ ${tagName} ] must be in lowercase.`, event.line, event.col, this, event.raw);
			});
		}
	};
}));
var require_tagname_specialchars = /* @__PURE__ */ __commonJSMin(((exports) => {
	Object.defineProperty(exports, "__esModule", { value: true });
	exports.default = {
		...
		init(parser, reporter) {
			...
			parser.addListener("tagstart,tagend", (event) => {
				const tagName = event.tagName;
				if (specialchars.test(tagName)) reporter.error(`The html element name of [ ${tagName} ] contains special character.`, event.line, event.col, this, event.raw);
			});
		}
	};
}));
var require_title_require = /* @__PURE__ */ __commonJSMin(((exports) => {
	Object.defineProperty(exports, "__esModule", { value: true });
	exports.default = {
		id: "title-require",
		...
		init(parser, reporter) {
			...
			const onTagStart = (event) => {
				const tagName = event.tagName.toLowerCase();
				if (tagName === "head") headBegin = true;
				else if (tagName === "title" && headBegin) hasTitle = true;
			};
			const onTagEnd = (event) => {
				const tagName = event.tagName.toLowerCase();
				if (hasTitle && tagName === "title") {
					const lastEvent = event.lastEvent;
					if (lastEvent.type !== "text" || lastEvent.type === "text" && /^\s*$/.test(lastEvent.raw) === true) reporter.error("<title></title> must not be empty.", event.line, event.col, this, event.raw);
				} else if (tagName === "head") {
					if (hasTitle === false) reporter.error("<title> must be present in <head> tag.", event.line, event.col, this, event.raw);
					parser.removeListener("tagstart", onTagStart);
					parser.removeListener("tagend", onTagEnd);
				}
			};
...
var require_tags_check = /* @__PURE__ */ __commonJSMin(((exports) => {
	Object.defineProperty(exports, "__esModule", { value: true });
	let tagsTypings = {
		a: {
			selfclosing: false,
		...
		script: { attrsOptional: [["async", "async"], ["defer", "defer"]] },
		img: {
			selfclosing: true,
			attrsRequired: [
				"src",
		...
		init(parser, reporter, options) {
			...
			parser.addListener("tagstart", (event) => {
				...
				if (tagsTypings[tagName]) {
					...
					if (currentTagType.selfclosing === true && !event.close) reporter.warn(`The <${tagName}> tag must be selfclosing.`, event.line, event.col, this, event.raw);
					else if (currentTagType.selfclosing === false && event.close) reporter.warn(`The <${tagName}> tag must not be selfclosing.`, event.line, event.col, this, event.raw);
					if (Array.isArray(currentTagType.attrsRequired)) currentTagType.attrsRequired.forEach((id) => {
						if (Array.isArray(id)) {
							const copyOfId = id.map((a) => a);
							const realID = copyOfId.shift();
							const values = copyOfId;
							if (attrs.some((attr) => attr.name === realID)) attrs.forEach((attr) => {
								if (attr.name === realID && values.indexOf(attr.value) === -1) reporter.error(`The <${tagName}> tag must have attr '${realID}' with one value of '${values.join("' or '")}'.`, event.line, col, this, event.raw);
							});
							else reporter.error(`The <${tagName}> tag must have attr '${realID}'.`, event.line, col, this, event.raw);
						} else if (!attrs.some((attr) => id.split("|").indexOf(attr.name) !== -1)) reporter.error(`The <${tagName}> tag must have attr '${id}'.`, event.line, col, this, event.raw);
					});
					if (Array.isArray(currentTagType.attrsOptional)) currentTagType.attrsOptional.forEach((id) => {
						if (Array.isArray(id)) {
							const copyOfId = id.map((a) => a);
							const realID = copyOfId.shift();
							const values = copyOfId;
							if (attrs.some((attr) => attr.name === realID)) attrs.forEach((attr) => {
								if (attr.name === realID && values.indexOf(attr.value) === -1) reporter.error(`The <${tagName}> tag must have optional attr '${realID}' with one value of '${values.join("' or '")}'.`, event.line, col, this, event.raw);
							});
						}
					});
					if (Array.isArray(currentTagType.redundantAttrs)) currentTagType.redundantAttrs.forEach((attrName) => {
						if (attrs.some((attr) => attr.name === attrName)) reporter.error(`The attr '${attrName}' is redundant for <${tagName}> and should be omitted.`, event.line, col, this, event.raw);
					});
				}
			});
		}
...
var require_attr_no_unnecessary_whitespace = /* @__PURE__ */ __commonJSMin(((exports) => {
	Object.defineProperty(exports, "__esModule", { value: true });
	exports.default = {
		...
		init(parser, reporter, options) {
			...
			parser.addListener("tagstart", (event) => {
				const attrs = event.attrs;
				const col = event.col + event.tagName.length + 1;
				for (let i = 0; i < attrs.length; i++) if (exceptions.indexOf(attrs[i].name) === -1) {
					const match = /(\s*)=(\s*)/.exec(attrs[i].raw.trim());
					if (match && (match[1].length !== 0 || match[2].length !== 0)) reporter.error(`The attribute '${attrs[i].name}' must not have spaces between the name and value.`, event.line, col + attrs[i].index, this, attrs[i].raw);
				}
			});
		}
	};
...
var import_core = (/* @__PURE__ */ __commonJSMin(((exports) => {
	...
	var HTMLHintCore = class {
		...
		verify(html, ruleset = this.defaultRuleset) {
			if (Object.keys(ruleset).length === 0) ruleset = this.defaultRuleset;
			html = html.replace(/^\s*<!--\s*htmlhint\s+([^\r\n]+?)\s*-->/i, (all, strRuleset) => {
				strRuleset.replace(/(?:^|,)\s*([^:,]+)\s*(?:\:\s*([^,\s]+))?/g, (all$1, ruleId, value) => {
			...
			for (const id in ruleset) {
				rule = rules[id];
				if (rule !== void 0 && ruleset[id] !== false) rule.init(parser, reporter, ruleset[id]);
			}
			parser.parse(html);
		...
		format(arrMessages, options = {}) {
			...
			arrMessages.forEach((hint) => {
				...
				if (rightCol < evidenceCount) evidence += "...";
				...
				if (match !== null) pointCol += match.length;
...
(function(ClientCapabilities$1) {
	ClientCapabilities$1.LATEST = { textDocument: {
		completion: { completionItem: { documentationFormat: [MarkupKind.Markdown, MarkupKind.PlainText] } },
...
var MultiLineStream = class {
	...
	advanceIfChars(ch) {
		let i;
		if (this.position + ch.length > this.source.length) return false;
		for (i = 0; i < ch.length; i++) if (this.source.charCodeAt(this.position + i) !== ch[i]) return false;
		this.advance(i);
		return true;
	}
	advanceIfRegExp(regex) {
		const match = this.source.substr(this.position).match(regex);
		if (match) {
	...
	advanceUntilChar(ch) {
		while (this.position < this.source.length) {
			if (this.source.charCodeAt(this.position) === ch) return true;
			this.advance(1);
		}
		return false;
	}
	...
	skipWhitespace() {
		return this.advanceWhileChar((ch) => {
			return ch === _WSP || ch === _TAB || ch === _NWL || ch === _LFD || ch === _CAR;
		}) > 0;
	}
	advanceWhileChar(condition) {
		const posNow = this.position;
		while (this.position < this.len && condition(this.source.charCodeAt(this.position))) this.position++;
...
function createScanner(input, initialOffset = 0, initialState = ScannerState.WithinContent, emitPseudoCloseTags = false) {
	...
	function internalScan() {
		const offset = stream.pos();
		if (stream.eos()) return finishToken(offset, TokenType.EOS);
		let errorMessage;
		switch (state) {
			...
			case ScannerState.AfterOpeningEndTag:
				...
				if (stream.skipWhitespace()) return finishToken(offset, TokenType.Whitespace, t("Tag name must directly follow the open bracket."));
				state = ScannerState.WithinEndTag;
				stream.advanceUntilChar(_RAN);
				if (offset < stream.pos()) return finishToken(offset, TokenType.Unknown, t("End tag name expected."));
				return internalScan();
			case ScannerState.WithinEndTag:
				if (stream.skipWhitespace()) return finishToken(offset, TokenType.Whitespace);
				if (stream.advanceIfChar(_RAN)) {
					state = ScannerState.WithinContent;
			...
			case ScannerState.AfterOpeningStartTag:
				...
				state = ScannerState.WithinTag;
				stream.advanceUntilChar(_RAN);
				if (offset < stream.pos()) return finishToken(offset, TokenType.Unknown, t("Start tag name expected."));
				return internalScan();
			case ScannerState.WithinTag:
				...
				if (stream.advanceIfChar(_RAN)) {
					if (lastTag === "script") if (lastTypeValue && htmlScriptContents[lastTypeValue]) state = ScannerState.WithinContent;
					else state = ScannerState.WithinScriptContent;
					else if (lastTag === "style") state = ScannerState.WithinStyleContent;
					else state = ScannerState.WithinContent;
			...
			case ScannerState.BeforeAttributeValue:
				...
				if (ch === _SQO || ch === _DQO) {
					stream.advance(1);
					if (stream.advanceUntilChar(ch)) stream.advance(1);
					if (lastAttributeName === "type") lastTypeValue = stream.getSource().substring(offset + 1, stream.pos() - 1);
					state = ScannerState.WithinTag;
					hasSpaceAfterTag = false;
					return finishToken(offset, TokenType.AttributeValue);
				}
			...
			case ScannerState.WithinScriptContent:
				...
				while (!stream.eos()) {
					...
					if (match.length === 0) {
						...
						if (sciptState === 1) sciptState = 2;
					} else if (match === "-->") sciptState = 1;
					else if (match[1] !== "/") {
						if (sciptState === 2) sciptState = 3;
					} else if (sciptState === 3) sciptState = 2;
					else {
				...
				state = ScannerState.WithinContent;
				if (offset < stream.pos()) return finishToken(offset, TokenType.Script);
				return internalScan();
			case ScannerState.WithinStyleContent:
				stream.advanceUntilRegExp(/<\/style/i);
				state = ScannerState.WithinContent;
				if (offset < stream.pos()) return finishToken(offset, TokenType.Styles);
				return internalScan();
		}
		stream.advance(1);
		state = ScannerState.WithinContent;
...
var Node = class {
	...
	isSameTag(tagInLowerCase) {
		if (this.tag === void 0) return tagInLowerCase === void 0;
		else return tagInLowerCase !== void 0 && this.tag.length === tagInLowerCase.length && this.tag.toLowerCase() === tagInLowerCase;
	}
	get firstChild() {
	...
	findNodeBefore(offset) {
		...
		if (idx >= 0) {
			...
			if (offset > child.start) {
				if (offset < child.end) return child.findNodeBefore(offset);
				const lastChild = child.lastChild;
				if (lastChild && lastChild.end === child.end) return child.findNodeBefore(offset);
				return child;
			}
		}
		return this;
	...
	findNodeAt(offset) {
		...
		if (idx >= 0) {
			const child = this.children[idx];
			if (offset > child.start && offset <= child.end) return child.findNodeAt(offset);
		}
		return this;
	}
};
		...
		while (token !== TokenType.EOS) {
			switch (token) {
				...
				case TokenType.EndTagClose:
					let node = curr;
					while (!node.isSameTag(endTagName) && node.parent) node = node.parent;
					if (node.parent) {
						while (curr !== node) {
				...
				case TokenType.AttributeName: {
					...
					if (!attributes) curr.attributes = attributes = {};
...
function getEntryBaselineImage(status) {
	...
	switch (status?.baseline) {
		...
		case "high":
			...
			break;
		default: baselineImg = BaselineImages.BASELINE_LIMITED;
	}
...
function getEntryBaselineStatus(status, browsers) {
	...
	if (status.baseline === false) {
		...
		let status$1 = `Limited availability across major browsers`;
		if (missingBrowsers) status$1 += ` (Not fully implemented in ${missingBrowsers})`;
		return status$1;
	}
...
function getMissingBaselineBrowsers(browsers) {
	...
	return missingBaselineBrowserFormatter.format(Object.values(Array.from(missingBrowsers.entries()).reduce((browsers$1, [browserId, browser]) => {
		if (browser.name in browsers$1 || browserId === "E") {
			browsers$1[browser.name] = browser.name;
			return browsers$1;
		}
		browsers$1[browser.name] = `${browser.name} on ${browser.platform}`;
		return browsers$1;
	}, {})));
}
var HTMLDataProvider = class {
	...
	constructor(id, customData) {
		...
		this._tags.forEach((t$1) => {
			this._tagMap[t$1.name.toLowerCase()] = t$1;
		});
		if (customData.valueSets) customData.valueSets.forEach((vs) => {
	...
	provideValues(tag, attribute) {
		...
		const processAttributes = (attributes) => {
			attributes.forEach((a) => {
				if (a.name.toLowerCase() === attribute) {
					if (a.values) a.values.forEach((v) => {
						values.push(v);
					});
					if (a.valueSet) {
						if (this._valueSetMap[a.valueSet]) this._valueSetMap[a.valueSet].forEach((v) => {
							values.push(v);
						});
					}
				}
		...
		const tagEntry = this._tagMap[tag.toLowerCase()];
		if (tagEntry) processAttributes(tagEntry.attributes);
...
function generateDocumentation(item, settings = {}, doesSupportMarkdown) {
	...
	if (item.status && settings.documentation !== false) {
		...
		if (doesSupportMarkdown) result.value += `${getEntryBaselineImage(item.status)} _${baselineStatus}_`;
		else result.value += baselineStatus;
	}
	if (item.references && item.references.length > 0 && settings.references !== false) {
		if (result.value.length) result.value += `\n\n`;
		if (doesSupportMarkdown) result.value += item.references.map((r) => {
			return `[${r.name}](${r.url})`;
		}).join(" | ");
		else result.value += item.references.map((r) => {
			return `${r.name}: ${r.url}`;
		}).join("\n");
	}
...
var PathCompletionParticipant = class {
	...
	onHtmlAttributeValue(context) {
		if (this.dataManager.isPathAttribute(context.tag, context.attribute)) this.atributeCompletions.push(context);
	}
...
function stripQuotes(fullValue) {
	if (startsWith(fullValue, `'`) || startsWith(fullValue, `"`)) return fullValue.slice(1, -1);
	else return fullValue;
}
function isCompletablePath(value) {
	if (startsWith(value, "http") || startsWith(value, "https") || startsWith(value, "//")) return false;
	return true;
}
function pathToReplaceRange(valueBeforeCursor, fullValue, range) {
	let replaceRange;
	const lastIndexOfSlash = valueBeforeCursor.lastIndexOf("/");
	if (lastIndexOfSlash === -1) replaceRange = shiftRange(range, 1, -1);
	else {
		const valueAfterLastSlash = fullValue.slice(lastIndexOfSlash + 1);
		const startPos = shiftPosition(range.end, -1 - valueAfterLastSlash.length);
		const whitespaceIndex = valueAfterLastSlash.indexOf(" ");
		let endPos;
		if (whitespaceIndex !== -1) endPos = shiftPosition(startPos, whitespaceIndex);
		else endPos = shiftPosition(range.end, -1);
...
var HTMLCompletion = class {
	...
	async doComplete2(document, position, htmlDocument, documentContext, settings) {
		if (!this.lsOptions.fileSystemProvider || !this.lsOptions.fileSystemProvider.readDirectory) return this.doComplete(document, position, htmlDocument, settings);
	...
	_doComplete(document, position, htmlDocument, settings) {
		...
		function getReplaceRange(replaceStart, replaceEnd = offset) {
			if (replaceStart > offset) replaceStart = offset;
			return {
		...
		function getLineIndent(offset$1) {
			let start = offset$1;
			while (start > 0) {
				const ch = text.charAt(start - 1);
				if ("\n\r".indexOf(ch) >= 0) return text.substring(start, offset$1);
		...
		const collectAutoCloseTagSuggestion = (tagCloseEnd, tag) => {
			if (settings && settings.hideAutoCompleteProposals) return result;
		...
		function collectAttributeNameSuggestions(nameStart, nameEnd = offset) {
			...
			while (replaceEnd < nameEnd && text[replaceEnd] !== "<") replaceEnd++;
			...
			if (!isFollowedBy(text, nameEnd, ScannerState.AfterAttributeName, TokenType.DelimiterAssign)) {
				...
				if (defaultValue === "empty") value = "=$1";
				else if (defaultValue === "singlequotes") value = "='$1'";
				else value = "=\"$1\"";
			}
		...
		function collectDataAttributesSuggestions(range, seenAttributes) {
			...
			function addNodeDataAttributes(node$1) {
				node$1.attributeNames.forEach((attr) => {
					if (startsWith(attr, dataAttr) && !dataAttributes[attr] && !seenAttributes[attr]) dataAttributes[attr] = attr + "=\"$1\"";
				});
				node$1.children.forEach((child) => addNodeDataAttributes(child));
			}
			if (htmlDocument) htmlDocument.roots.forEach((root) => addNodeDataAttributes(root));
		...
		function collectAttributeValueSuggestions(valueStart, valueEnd = offset) {
			...
			if (offset > valueStart && offset <= valueEnd && isQuote(text[valueStart])) {
				...
				if (valueEnd > valueStart && text[valueEnd - 1] === text[valueStart]) valueContentEnd--;
				range = getReplaceRange(getWordStart(text, offset, valueContentStart), getWordEnd(text, offset, valueContentEnd));
			...
			if (completionParticipants.length > 0) {
				...
				for (const participant of completionParticipants) if (participant.onHtmlAttributeValue) participant.onHtmlAttributeValue({
					document,
					position,
					tag,
					attribute,
		...
		function scanNextForEndPos(nextToken) {
			if (offset === scanner.getTokenEnd()) {
				token = scanner.scan();
				if (token === nextToken && scanner.getTokenOffset() === offset) return scanner.getTokenEnd();
			}
			return offset;
		}
		function collectInsideContent() {
			for (const participant of completionParticipants) if (participant.onHtmlContent) participant.onHtmlContent({
				document,
				position
			});
		...
		function collectCharacterEntityProposals() {
			...
			if (k >= 0 && text[k] === "&") {
				...
				for (const entity in entities) if (endsWith(entity, ";")) {
					const label = "&" + entity;
					result.items.push({
						label,
			...
			switch (token) {
				case TokenType.StartTagOpen:
					if (scanner.getTokenEnd() === offset) {
						...
						if (position.line === 0) suggestDoctype(offset, endPos);
						return collectTagSuggestions(offset, endPos);
					}
					break;
				case TokenType.StartTag:
					if (scanner.getTokenOffset() <= offset && offset <= scanner.getTokenEnd()) return collectOpenTagSuggestions(scanner.getTokenOffset(), scanner.getTokenEnd());
					currentTag = scanner.getTokenText();
					break;
				case TokenType.AttributeName:
					if (scanner.getTokenOffset() <= offset && offset <= scanner.getTokenEnd()) return collectAttributeNameSuggestions(scanner.getTokenOffset(), scanner.getTokenEnd());
					currentAttributeName = scanner.getTokenText();
					break;
				case TokenType.DelimiterAssign:
					if (scanner.getTokenEnd() === offset) return collectAttributeValueSuggestions(offset, scanNextForEndPos(TokenType.AttributeValue));
					break;
				case TokenType.AttributeValue:
					if (scanner.getTokenOffset() <= offset && offset <= scanner.getTokenEnd()) return collectAttributeValueSuggestions(scanner.getTokenOffset(), scanner.getTokenEnd());
					break;
				case TokenType.Whitespace:
					if (offset <= scanner.getTokenEnd()) switch (scanner.getScannerState()) {
						case ScannerState.AfterOpeningStartTag: return collectTagSuggestions(scanner.getTokenOffset(), scanNextForEndPos(TokenType.StartTag));
						case ScannerState.WithinTag:
						case ScannerState.AfterAttributeName: return collectAttributeNameSuggestions(scanner.getTokenEnd());
						case ScannerState.BeforeAttributeValue: return collectAttributeValueSuggestions(scanner.getTokenEnd());
						case ScannerState.AfterOpeningEndTag: return collectCloseTagSuggestions(scanner.getTokenOffset() - 1, false);
						case ScannerState.WithinContent: return collectInsideContent();
					}
					break;
				case TokenType.EndTagOpen:
					if (offset <= scanner.getTokenEnd()) return collectCloseTagSuggestions(scanner.getTokenOffset() + 1, false, scanNextForEndPos(TokenType.EndTag));
					break;
				case TokenType.EndTag:
					if (offset <= scanner.getTokenEnd()) {
						...
						while (start >= 0) {
							...
							if (ch === "/") return collectCloseTagSuggestions(start, false, scanner.getTokenEnd());
				...
				case TokenType.StartTagClose:
					if (offset <= scanner.getTokenEnd()) {
						if (currentTag) return collectAutoCloseTagSuggestion(scanner.getTokenEnd(), currentTag);
					}
					break;
	...
	doQuoteComplete(document, position, htmlDocument, settings) {
		...
		const defaultValue = settings?.attributeDefaultValue ?? "doublequotes";
		if (defaultValue === "empty") return null;
		...
		if (node && node.attributes && node.start < offset && (!node.endTagStart || node.endTagStart > offset)) {
			...
			while (token !== TokenType.EOS && scanner.getTokenEnd() <= offset) {
				if (token === TokenType.AttributeName && scanner.getTokenEnd() === offset - 1) {
					token = scanner.scan();
					if (token !== TokenType.DelimiterAssign) return null;
					token = scanner.scan();
					if (token === TokenType.Unknown || token === TokenType.AttributeValue) return null;
					return value;
				}
				token = scanner.scan();
			}
		...
		if (char === ">") {
			...
			if (node && node.tag && node.start < offset && (!node.endTagStart || node.endTagStart > offset)) {
				...
				if (!this.dataManager.isVoidElement(node.tag, voidElements)) {
					...
					let token = scanner.scan();
					while (token !== TokenType.EOS && scanner.getTokenEnd() <= offset) {
						if (token === TokenType.StartTagClose && scanner.getTokenEnd() === offset) return `$0</${node.tag}>`;
						token = scanner.scan();
					}
				}
			}
			...
			while (node && node.closed && !(node.endTagStart && node.endTagStart > offset)) node = node.parent;
			if (node && node.tag) {
				const scanner = createScanner(document.getText(), node.start);
				let token = scanner.scan();
				while (token !== TokenType.EOS && scanner.getTokenEnd() <= offset) {
					if (token === TokenType.EndTagOpen && scanner.getTokenEnd() === offset) if (document.getText().charAt(offset) !== ">") return `${node.tag}>`;
	...
	convertCompletionList(list) {
		if (!this.doesSupportMarkdown()) list.items.forEach((item) => {
			if (item.documentation && typeof item.documentation !== "string") item.documentation = {
				kind: "plaintext",
...
function isFollowedBy(s, offset, intialState, expectedToken) {
	...
	let token = scanner.scan();
	while (token === TokenType.Whitespace) token = scanner.scan();
	return token === expectedToken;
}
function getWordStart(s, offset, limit) {
	while (offset > limit && !isWhiteSpace(s[offset - 1])) offset--;
	return offset;
}
function getWordEnd(s, offset, limit) {
	while (offset < limit && !isWhiteSpace(s[offset])) offset++;
	return offset;
}
var HTMLHover = class {
	constructor(lsOptions, dataManager) {
	...
	doHover(document, position, htmlDocument, options) {
		...
		function getEntityHover(text$1, range) {
			let currEntity = filterEntity(text$1);
		...
		function getTagNameRange$1(tokenType, startOffset) {
			const scanner = createScanner(document.getText(), startOffset);
			let token = scanner.scan();
			while (token !== TokenType.EOS && (scanner.getTokenEnd() < offset || scanner.getTokenEnd() === offset && token !== tokenType)) token = scanner.scan();
			if (token === tokenType && offset <= scanner.getTokenEnd()) return {
		...
		function getEntityRange() {
			...
			if (k >= 0 && text[k] === "&") {
				let range = null;
				if (text[n] === ";") range = Range.create(Position.create(position.line, characterStart), Position.create(position.line, characterEnd + 1));
				else range = Range.create(Position.create(position.line, characterStart), Position.create(position.line, characterEnd));
				return range;
			}
			return null;
		}
		function filterEntity(text$1) {
			let k = offset - 1;
			let newText = "&";
			while (k >= 0 && isLetterOrDigit(text$1, k)) k--;
			k = k + 1;
			while (isLetterOrDigit(text$1, k)) {
				newText += text$1[k];
				k += 1;
			}
			newText += ";";
		...
		if (node.endTagStart && offset >= node.endTagStart) {
			const tagRange$1 = getTagNameRange$1(TokenType.EndTag, node.endTagStart);
			if (tagRange$1) return getTagHover(node.tag, tagRange$1, false);
			return null;
		}
		const tagRange = getTagNameRange$1(TokenType.StartTag, node.start);
		if (tagRange) return getTagHover(node.tag, tagRange, true);
		const attrRange = getTagNameRange$1(TokenType.AttributeName, node.start);
		if (attrRange) {
			const tag = node.tag;
			return getAttrHover(tag, document.getText(attrRange), attrRange);
		}
		const entityRange = getEntityRange();
		if (entityRange) return getEntityHover(text, entityRange);
		function scanAttrAndAttrValue(nodeStart, attrValueStart) {
			...
			while (token !== TokenType.EOS && scanner.getTokenEnd() <= attrValueStart) {
				token = scanner.scan();
				if (token === TokenType.AttributeName) prevAttr = scanner.getTokenText();
			}
			return prevAttr;
		}
		const attrValueRange = getTagNameRange$1(TokenType.AttributeValue, node.start);
		if (attrValueRange) {
			const tag = node.tag;
			...
			if (matchAttr) return getAttrValueHover(tag, matchAttr, attrValue, attrValueRange);
		}
		return null;
	}
	convertContents(contents) {
		if (!this.doesSupportMarkdown()) if (typeof contents === "string") return contents;
		else if ("kind" in contents) return {
			kind: "plaintext",
			value: contents.value
		};
		else if (Array.isArray(contents)) contents.map((c) => {
			return typeof c === "string" ? c : c.value;
		});
		else return contents.value;
		return contents;
	}
	doesSupportMarkdown() {
		if (!isDefined(this.supportsMarkdown)) {
...
function trimQuotes(s) {
	...
	if (s[s.length - 1] === `'` || s[s.length - 1] === `"`) s = s.slice(0, -1);
	return s;
}
...
(function() {
	...
	var __webpack_modules__ = [
		...
		,
		(function(module, __unused_webpack_exports, __webpack_require__$1) {
			var Beautifier = __webpack_require__$1(16).Beautifier, Options = __webpack_require__$1(17).Options;
			function css_beautify$1(source_text, options) {
				return new Beautifier(source_text, options).beautify();
			}
			module.exports = css_beautify$1;
			module.exports.defaultOptions = function() {
				return new Options();
			};
		}),
		(function(module, __unused_webpack_exports, __webpack_require__$1) {
			var Options = __webpack_require__$1(17).Options;
			var Output = __webpack_require__$1(2).Output;
			var InputScanner = __webpack_require__$1(8).InputScanner;
			var Directives = __webpack_require__$1(13).Directives;
		...
		(function(module, __unused_webpack_exports, __webpack_require__$1) {
			var BaseOptions = __webpack_require__$1(6).Options;
	...
	legacy_beautify_css = __webpack_require__(15);
})();
...
(function() {
	...
	var __webpack_modules__ = [
		...
		(function(module, __unused_webpack_exports, __webpack_require__$1) {
			var InputScanner = __webpack_require__$1(8).InputScanner;
			var Token = __webpack_require__$1(3).Token;
			var TokenStream = __webpack_require__$1(10).TokenStream;
			var WhitespacePattern = __webpack_require__$1(11).WhitespacePattern;
			var TOKEN = {
				START: "TK_START",
				RAW: "TK_RAW",
				EOF: "TK_EOF"
		...
		(function(module, __unused_webpack_exports, __webpack_require__$1) {
			var Pattern = __webpack_require__$1(12).Pattern;
		...
		,
		(function(module, __unused_webpack_exports, __webpack_require__$1) {
			var Beautifier = __webpack_require__$1(19).Beautifier, Options = __webpack_require__$1(20).Options;
			function style_html(html_source, options, js_beautify$1, css_beautify$1) {
				return new Beautifier(html_source, options, js_beautify$1, css_beautify$1).beautify();
			}
		...
		(function(module, __unused_webpack_exports, __webpack_require__$1) {
			var Options = __webpack_require__$1(20).Options;
			var Output = __webpack_require__$1(2).Output;
			var Tokenizer = __webpack_require__$1(21).Tokenizer;
			var TOKEN = __webpack_require__$1(21).TOKEN;
			var lineBreak = /\r\n|[\r\n]/;
			var allLineBreaks = /\r\n|[\r\n]/g;
			...
			function Beautifier(source_text, options, js_beautify$1, css_beautify$1) {
				this._source_text = source_text || "";
				options = options || {};
				this._js_beautify = js_beautify$1;
				this._css_beautify = css_beautify$1;
		...
		(function(module, __unused_webpack_exports, __webpack_require__$1) {
			var BaseTokenizer = __webpack_require__$1(9).Tokenizer;
			var BASETOKEN = __webpack_require__$1(9).TOKEN;
			var Directives = __webpack_require__$1(13).Directives;
			var TemplatablePattern = __webpack_require__$1(14).TemplatablePattern;
			var Pattern = __webpack_require__$1(12).Pattern;
			var TOKEN = {
	...
	legacy_beautify_html = __webpack_require__(18);
})();
...
function format(document, range, options) {
	...
	if (range) {
		...
		while (extendedStart > 0 && isWhitespace(value, extendedStart - 1)) extendedStart--;
		if (extendedStart === 0 || isEOL(value, extendedStart - 1)) startOffset = extendedStart;
		else if (extendedStart < startOffset) startOffset = extendedStart + 1;
		let endOffset = document.offsetAt(range.end);
		let extendedEnd = endOffset;
		while (extendedEnd < value.length && isWhitespace(value, extendedEnd)) extendedEnd++;
		if (extendedEnd === value.length || isEOL(value, extendedEnd)) endOffset = extendedEnd;
		range = Range.create(document.positionAt(startOffset), document.positionAt(endOffset));
		const firstHalf = value.substring(0, startOffset);
		if ((/* @__PURE__ */ new RegExp(/.*[<][^>]*$/)).test(firstHalf)) {
	...
	} else range = Range.create(Position.create(0, 0), document.positionAt(value.length));
	const htmlOptions = {
		indent_size: tabSize,
	...
	if (initialIndentLevel > 0) {
		...
		result = result.split("\n").join("\n" + indent);
		if (range.start.character === 0) result = indent + result;
	}
	return [{
		range,
		newText: result
...
function getTagsFormatOption(options, key, dflt) {
	...
	if (typeof list === "string") {
		if (list.length > 0) return list.split(",").map((t$1) => t$1.trim().toLowerCase());
		return [];
	}
	return dflt;
}
...
function normalizeRef(url) {
	const first = url[0];
	if (first === url[url.length - 1] && (first === "'" || first === "\"")) url = url.substring(1, url.length - 1);
	return url;
}
function validateRef(url, languageId) {
	if (!url.length) return false;
	if (languageId === "handlebars" && /{{|}}/.test(url)) return false;
	return /\b(w[\w\d+.-]*:\/\/)?[^\s()<>]+(?:\([\w\d]+\)|([^[:punct:]\s]|\/?))/.test(url);
}
function getWorkspaceUrl(documentUri, tokenContent, documentContext, base) {
	if (/^\s*javascript\:/i.test(tokenContent) || /[\n\r]/.test(tokenContent)) return;
	tokenContent = tokenContent.replace(/^\s*/g, "");
	const match = tokenContent.match(/^(\w[\w\d+.-]*):/);
	if (match) {
		const schema = match[1].toLowerCase();
		if (schema === "http" || schema === "https" || schema === "file") return tokenContent;
		return;
	}
	if (/^\#/i.test(tokenContent)) return documentUri + tokenContent;
	if (/^\/\//i.test(tokenContent)) return (startsWith(documentUri, "https://") ? "https" : "http") + ":" + tokenContent.replace(/^\s*/g, "");
	if (documentContext) return documentContext.resolveReference(tokenContent, base || documentUri);
	return tokenContent;
}
function createLink(document, documentContext, attributeValue, startOffset, endOffset, base) {
	...
	if (!validateRef(tokenContent, document.languageId)) return;
...
function validateAndCleanURI(uriStr, document) {
	try {
		...
		if (uri.scheme === "file" && uri.fragment && !(uriStr.startsWith(document.uri) && uriStr.charCodeAt(document.uri.length) === _hash)) return uri.with({ fragment: null }).toString(true);
		return uriStr;
	} catch (e) {
		return;
	}
	...
	findDocumentLinks(document, documentContext) {
		...
		while (token !== TokenType.EOS) {
			switch (token) {
				case TokenType.StartTag:
					...
					if (!base) afterBase = lastTagName === "base";
					break;
				case TokenType.AttributeName:
		...
		for (const link of newLinks) {
			...
			if (link.target && startsWith(link.target, localWithHash)) {
				const offset = idLocations[link.target.substring(localWithHash.length)];
				if (offset !== void 0) {
					const pos = document.positionAt(offset);
					link.target = `${localWithHash}${pos.line + 1},${pos.character + 1}`;
				} else link.target = document.uri;
			}
		}
		return newLinks;
	}
...
function findDocumentHighlights(document, position, htmlDocument) {
	...
	if (startTagRange && covers(startTagRange, position) || endTagRange && covers(endTagRange, position)) {
		if (startTagRange) result.push({
			kind: DocumentHighlightKind.Read,
			range: startTagRange
		});
		if (endTagRange) result.push({
			kind: DocumentHighlightKind.Read,
			range: endTagRange
		});
	}
...
function getTagNameRange(tokenType, document, startOffset) {
	const scanner = createScanner(document.getText(), startOffset);
	let token = scanner.scan();
	while (token !== TokenType.EOS && token !== tokenType) token = scanner.scan();
	if (token !== TokenType.EOS) return {
		start: document.positionAt(scanner.getTokenOffset()),
		end: document.positionAt(scanner.getTokenEnd())
	};
	return null;
...
function findDocumentSymbols(document, htmlDocument) {
	...
	for (const symbol of symbols2) walk(symbol, void 0);
	return symbols;
...
function nodeToName(node) {
	...
	if (node.attributes) {
		...
		if (id) name += `#${id.replace(/[\"\']/g, "")}`;
		if (classes) name += classes.replace(/[\"\']/g, "").split(/\s+/).map((className) => `.${className}`).join("");
	}
	return name || "?";
}
function doRename(document, position, newName, htmlDocument) {
	...
	if (!isWithinTagRange(node, offset, node.tag)) return null;
	const edits = [];
	const startTagRange = {
		start: document.positionAt(node.start + 1),
		end: document.positionAt(node.start + 1 + node.tag.length)
	};
	edits.push({
		range: startTagRange,
		newText: newName
	...
	if (node.endTagStart) {
		const endTagRange = {
			start: document.positionAt(node.endTagStart + 2),
			end: document.positionAt(node.endTagStart + 2 + node.tag.length)
		};
		edits.push({
			range: endTagRange,
			newText: newName
...
function isWithinTagRange(node, offset, nodeTag) {
	if (node.endTagStart) {
		if (node.endTagStart + 2 <= offset && offset <= node.endTagStart + 2 + nodeTag.length) return true;
	}
	return node.start + 1 <= offset && offset <= node.start + 1 + nodeTag.length;
}
function findMatchingTagPosition(document, position, htmlDocument) {
	const offset = document.offsetAt(position);
	const node = htmlDocument.findNodeAt(offset);
	if (!node.tag) return null;
	if (!node.endTagStart) return null;
	if (node.start + 1 <= offset && offset <= node.start + 1 + node.tag.length) {
		const mirrorOffset = offset - 1 - node.start + node.endTagStart + 2;
		return document.positionAt(mirrorOffset);
	}
	if (node.endTagStart + 2 <= offset && offset <= node.endTagStart + 2 + node.tag.length) {
		const mirrorOffset = offset - 2 - node.endTagStart + node.start + 1;
		return document.positionAt(mirrorOffset);
	}
	return null;
}
function findLinkedEditingRanges(document, position, htmlDocument) {
	...
	if (!node.endTagStart) return null;
	if (node.start + 1 <= offset && offset <= node.start + 1 + tagLength || node.endTagStart + 2 <= offset && offset <= node.endTagStart + 2 + tagLength) return [Range.create(document.positionAt(node.start + 1), document.positionAt(node.start + 1 + tagLength)), Range.create(document.positionAt(node.endTagStart + 2), document.positionAt(node.endTagStart + 2 + tagLength))];
	return null;
}
var HTMLFolding = class {
	...
	limitRanges(ranges, rangeLimit) {
		ranges = ranges.sort((r1, r2) => {
			...
			if (diff === 0) diff = r1.endLine - r2.endLine;
			return diff;
		});
		let top = void 0;
		const previous = [];
		...
		const setNestingLevel = (index, level) => {
			...
			if (level < 30) nestingLevelCounts[level] = (nestingLevelCounts[level] || 0) + 1;
		};
	...
	getFoldingRanges(document, context) {
		...
		while (token !== TokenType.EOS) {
			switch (token) {
				...
				case TokenType.EndTag:
					...
					break;
				case TokenType.StartTagClose:
					if (!lastTagName) break;
					voidElements ?? (voidElements = this.dataManager.getVoidElements(document.languageId));
					if (!this.dataManager.isVoidElement(lastTagName, voidElements)) break;
				case TokenType.EndTagClose:
				case TokenType.StartTagSelfClose: {
					let i = stack.length - 1;
					while (i >= 0 && stack[i].tagName !== lastTagName) i--;
					if (i >= 0) {
						...
						if (endLine > startLine && prevStart !== startLine) addRange({
							startLine,
							endLine
						});
					}
				...
				case TokenType.Comment: {
					...
					const m = scanner.getTokenText().match(/^\s*#(region\b)|(endregion\b)/);
					if (m) if (m[1]) stack.push({
						startLine,
						tagName: ""
					});
					else {
						let i = stack.length - 1;
						while (i >= 0 && stack[i].tagName.length) i--;
						if (i >= 0) {
							const stackElement = stack[i];
							stack.length = i;
		...
		if (ranges.length > rangeLimit) return this.limitRanges(ranges, rangeLimit);
		return ranges;
	}
};
var HTMLSelectionRange = class {
	...
	getSelectionRange(position, document, htmlDocument) {
		...
		for (let index = applicableRanges.length - 1; index >= 0; index--) {
			...
			if (!prev || range[0] !== prev[0] || range[1] !== prev[1]) current = SelectionRange.create(Range.create(document.positionAt(applicableRanges[index][0]), document.positionAt(applicableRanges[index][1])), current);
			prev = range;
		}
	...
	getApplicableRanges(document, position, htmlDoc) {
		...
		if (currNode.startTagEnd && !currNode.endTagStart) {
			if (currNode.startTagEnd !== currNode.end) return [[currNode.start, currNode.end]];
			const closeRange = Range.create(document.positionAt(currNode.startTagEnd - 2), document.positionAt(currNode.startTagEnd));
			if (document.getText(closeRange) === "/>") result.unshift([currNode.start + 1, currNode.startTagEnd - 2]);
			else result.unshift([currNode.start + 1, currNode.startTagEnd - 1]);
			result = this.getAttributeLevelRanges(document, currNode, currOffset).concat(result);
			return result;
		}
		if (!currNode.startTagEnd || !currNode.endTagStart) return result;
		...
		if (currNode.start < currOffset && currOffset < currNode.startTagEnd) {
			...
			if (currOffset >= currNode.endTagStart + 2) result.unshift([currNode.endTagStart + 2, currNode.end - 1]);
			return result;
		}
	}
	...
	getNodeRanges(n) {
		if (n.startTagEnd && n.endTagStart && n.startTagEnd < n.endTagStart) return [[n.startTagEnd, n.endTagStart], [n.start, n.end]];
		...
		while (token !== TokenType.EOS) {
			switch (token) {
				case TokenType.AttributeName:
					...
					if (relativeOffset <= scanner.getTokenEnd()) result.unshift([scanner.getTokenOffset(), scanner.getTokenEnd()]);
					isInsideAttribute = true;
					attrStart = scanner.getTokenOffset();
					break;
				case TokenType.AttributeValue: {
					if (!isInsideAttribute) break;
					...
					if (relativeOffset >= scanner.getTokenOffset() && relativeOffset <= scanner.getTokenEnd()) {
						...
						if (valueText[0] === `"` && valueText[valueText.length - 1] === `"` || valueText[0] === `'` && valueText[valueText.length - 1] === `'`) {
							if (relativeOffset >= scanner.getTokenOffset() + 1 && relativeOffset <= scanner.getTokenEnd() - 1) result.unshift([scanner.getTokenOffset() + 1, scanner.getTokenEnd() - 1]);
						}
						result.push([attrStart, scanner.getTokenEnd()]);
					}
					break;
...
var HTMLDataManager = class {
	...
	setDataProviders(builtIn, providers) {
		this.dataProviders = [];
		if (builtIn) this.dataProviders.push(new HTMLDataProvider("html5", htmlData));
	...
	isPathAttribute(tag, attr) {
		if (attr === "src" || attr === "href") return true;
		const a = PATH_TAG_AND_ATTR[tag];
		if (a) if (typeof a === "string") return a === attr;
		else return a.indexOf(attr) !== -1;
		return false;
	}
};
...
function toDiagnostics(diagnostics, filterErrors) {
	return CommonConverter.excludeByErrorMessage(diagnostics, filterErrors.errorMessagesToIgnore).map((el) => {
		let severity;
		if (checkValueAgainstRegexpArray(el.message, filterErrors.errorMessagesToTreatAsWarning)) severity = 2;
		else if (checkValueAgainstRegexpArray(el.message, filterErrors.errorMessagesToTreatAsInfo)) severity = 3;
		else severity = el.type === "error" ? 1 : el.type === "warning" ? 2 : 3;
		return {
			range: {
				start: {
					line: el.line - 1,
...
var HtmlService = class extends BaseService {
	constructor(mode) {
		super(mode);
	...
	async doValidation(document) {
		...
		return toDiagnostics(import_core.HTMLHint.verify(fullDocument.getText(), options), this.optionsToFilterDiagnostics);
	}

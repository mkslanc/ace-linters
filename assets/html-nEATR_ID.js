...
import { n as require_xml$1, t as require_xml } from "./xml--3d0d31h.js";
...
import { t as require_mixed } from "./mixed-B6aEv4ic.js";
var require_html$1 = /* @__PURE__ */ __commonJSMin(((exports) => {
	...
	var FoldMode = exports.FoldMode = function(voidElements$1, optionalTags) {
		MixedFoldMode.call(this, new XmlFoldMode(voidElements$1, optionalTags), {
			"js-": new CStyleFoldMode(),
			"css-": new CStyleFoldMode()
		});
	};
	oop$1.inherits(FoldMode, MixedFoldMode);
}));
var require_html_completions = /* @__PURE__ */ __commonJSMin(((exports) => {
	...
	function findTagName(session, pos) {
		...
		while (token && !is(token, "tag-name")) token = iterator.stepBackward();
		if (token) return token.value;
	}
	function findAttributeName(session, pos) {
		var iterator = new TokenIterator(session, pos.row, pos.column);
		var token = iterator.getCurrentToken();
		while (token && !is(token, "attribute-name")) token = iterator.stepBackward();
		if (token) return token.value;
	}
	var HtmlCompletions$1 = function() {};
	(function() {
		...
		this.getAttributeCompletions = function(state, session, pos, prefix) {
			...
			if (tagName in attributeMap) attributes = attributes.concat(Object.keys(attributeMap[tagName]));
		...
		this.getAttributeValueCompletions = function(state, session, pos, prefix) {
			...
			if (!tagName) return [];
			var values = [];
			if (tagName in attributeMap && attributeName in attributeMap[tagName] && typeof attributeMap[tagName][attributeName] === "object") values = Object.keys(attributeMap[tagName][attributeName]);
	...
	}).call(HtmlCompletions$1.prototype);
	exports.HtmlCompletions = HtmlCompletions$1;
}));
...
export { require_html_completions as n, require_html$1 as r, require_html as t };

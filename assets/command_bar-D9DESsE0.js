...
//#region node_modules/ace-code/src/ext/command_bar.js
var require_command_bar = /* @__PURE__ */ __commonJSMin(((exports) => {
	/**
	...
	*/
	...
	var minPosition = function(posA, posB) {
		if (posB.row > posA.row) return posA;
		else if (posB.row === posA.row && posB.column > posA.column) return posA;
		return posB;
	};
	var keyDisplayMap = {
		"Ctrl": { mac: "^" },
	...
	var CommandBarTooltip = class {
		...
		*/
		registerCommand(id, command) {
			...
			if (!registerForMainTooltip && !this.elements[MORE_OPTIONS_BUTTON_ID]) this.$createCommand(MORE_OPTIONS_BUTTON_ID, {
				name: "···",
				exec: function() {
			...
			if (this.isShown()) this.updatePosition();
		}
		isShown() {
		...
		*/
		attach(editor) {
			if (!editor || this.isShown() && this.editor === editor) return;
			this.detach();
			this.editor = editor;
			...
			if (this.getAlwaysShow()) this.$showTooltip();
			else this.$updateOnHoverHandlers(true);
		}
		/**
		...
		*/
		updatePosition() {
			...
			var ranges;
			if (this.editor.selection.getAllRanges) ranges = this.editor.selection.getAllRanges();
			else ranges = [this.editor.getSelectionRange()];
			if (!ranges.length) return;
			var minPos = minPosition(ranges[0].start, ranges[0].end);
			for (var i = 0, range; range = ranges[i]; i++) minPos = minPosition(minPos, minPosition(range.start, range.end));
			...
			if (!(top >= 0 && top + tooltipEl.offsetHeight <= screenHeight && left >= 0 && left + tooltipEl.offsetWidth <= screenWidth)) {
				this.$hideTooltip();
				return;
			}
			...
			if (this.isMoreOptionsShown()) {
				...
				var screenHeight = window.innerHeight;
				if (top + moreOptionsEl.offsetHeight > screenHeight) top -= tooltipEl.offsetHeight + moreOptionsEl.offsetHeight;
				if (left + moreOptionsEl.offsetWidth > screenWidth) left = screenWidth - moreOptionsEl.offsetWidth;
		...
		*/
		$createCommand(id, command, forMainTooltip) {
			...
			if (bindKey) {
				if (typeof bindKey === "object") bindKey = useragent.isMac ? bindKey.mac : bindKey.win;
				bindKey = bindKey.split("|")[0];
				keyParts = bindKey.split("-");
				keyParts = keyParts.map(function(key) {
					if (keyDisplayMap[key]) {
						if (typeof keyDisplayMap[key] === "string") return keyDisplayMap[key];
						else if (useragent.isMac && keyDisplayMap[key].mac) return keyDisplayMap[key].mac;
					}
					return key;
				});
			}
			/**@type {any[]} */
			var buttonNode;
			if (forMainTooltip && command.iconCssClass) buttonNode = ["div", {
				class: ["ace_icon_svg", command.iconCssClass].join(" "),
				"aria-label": command.name + " (" + command.bindKey + ")"
			}];
			else {
				buttonNode = [["div", { class: VALUE_CLASS_NAME }], [
					"div",
					{ class: CAPTION_CLASS_NAME },
					command.name
				]];
				if (keyParts.length) buttonNode.push([
					"div",
			...
			var eventListener = function(e) {
				...
				if (!this.elements[id].disabled && command.exec) command.exec(this.editor);
				if (this.$shouldHideMoreOptions) this.$setMoreOptionsVisibility(false);
				this.update();
				e.preventDefault();
			}.bind(this);
		...
		*/
		...
		$onChangeScroll() {
			if (this.editor.renderer && (this.isShown() || this.getAlwaysShow())) this.editor.renderer.once("afterRender", this.updatePosition.bind(this));
		}
		$onMouseMove(e) {
			...
			if (e.clientY >= cursorScreenPosition.pageY && e.clientY < cursorScreenPosition.pageY + lineHeight) {
				if (!this.isShown() && !this.$showTooltipTimer.isPending()) this.$showTooltipTimer.delay();
				if (this.$hideTooltipTimer.isPending()) this.$hideTooltipTimer.cancel();
			} else {
				if (this.isShown() && !this.$hideTooltipTimer.isPending()) this.$hideTooltipTimer.delay();
				if (this.$showTooltipTimer.isPending()) this.$showTooltipTimer.cancel();
			}
		}
		$preventMouseEvent(e) {
			if (this.editor) this.editor.focus();
		...
		*/
		$updateElement(id) {
			...
			if (typeof commandEnabled === "function") commandEnabled = commandEnabled(this.editor);
			if (typeof command.getValue === "function") {
				var value = command.getValue(this.editor);
				if (command.type === "text") el.textContent = value;
					...
					if (isOnTooltip) domCssFn(el, "ace_selected");
					else {
...
//#endregion
export { require_command_bar as t };

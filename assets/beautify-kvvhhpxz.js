...
//#region node_modules/ace-code/src/ext/beautify.js
/**
...
* @module
*/
var require_beautify = /* @__PURE__ */ __commonJSMin(((exports) => {
	...
	*/
	exports.formatOptions = { lineBreaksAfterCommasInCurlyBlock: true };
	/**
	...
	exports.beautify = function(session) {
		...
		while (token !== null) {
			...
			if (typeof token !== "undefined") {
				...
				} else if (is(token, "tag-close")) inTag = false;
				else if (is(token, "comment.start")) inBlock = true;
				else if (is(token, "comment.end")) inBlock = false;
				if (!inTag && !rowsToAdd && token.type === "paren.rparen" && token.value.substr(0, 1) === "}") rowsToAdd++;
				if (curRow !== row) {
					rowsToAdd = curRow;
				...
				if (value) {
					if (token.type === "keyword" && value.match(/^(if|else|elseif|for|foreach|while|switch)$/)) {
						...
						if (value.substr(0, 1) === "{") {
							...
							if (code.substr(-1) !== "[" && code.trimRight().substr(-1) === "[") {
								trimCode();
								spaceBefore = false;
							} else if (code.trimRight().substr(-1) === ")") trimCode();
							else trimLine();
						}
						...
						if (value.substr(0, 1) === "}") {
							...
							if (code.trimRight().substr(-1) === "{") trimCode();
							else {
								spaceBefore = true;
						...
						trimCode();
						trimNext();
						if (value.match(/^(,)$/) && curlyDepth > 0 && roundDepth === 0 && formatOptions.lineBreaksAfterCommasInCurlyBlock) rowsToAdd++;
						else {
							spaceAfter = true;
							breakBefore = false;
						}
						...
						trimCode();
						spaceBefore = true;
					} else if (is(token, "attribute-name") && code.substr(-1).match(/^\s$/)) spaceBefore = true;
					...
					if (token.type === "paren.rparen") {
						...
						for (i = 0; i < value.length; i++) {
							depth--;
							if (value.substr(i, 1) === "}" && parents[depth] === "case") depth--;
						}
					}
					...
					spaceBefore = false;
					spaceAfter = false;
					if (is(token, "tag-close") && (inBlock || blockTags.indexOf(tagName) !== -1) || is(token, "doctype") && value === ">") if (inBlock && nextToken && nextToken.value === "</") rowsToAdd = -1;
					else rowsToAdd = 1;
					if (nextToken && singletonTags.indexOf(nextToken.value) === -1) {
						if (is(token, "tag-open") && value === "</") depth--;
						else if (is(token, "tag-open") && value === "<") depth++;
						else if (is(token, "tag-close") && value === "/>") depth--;
					}
					if (is(token, "tag-name")) tagName = value;
					row = curRow;
				}
			}
			token = nextToken;

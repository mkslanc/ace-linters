...
var require_csound_preprocessor_highlight_rules = /* @__PURE__ */ __commonJSMin(((exports) => {
	var oop$1 = require_oop();
	var TextHighlightRules = require_text_highlight_rules().TextHighlightRules;
	var CsoundPreprocessorHighlightRules$1 = function(embeddedRulePrefix) {
	...
	oop$1.inherits(CsoundPreprocessorHighlightRules$1, TextHighlightRules);
	(function() {
		this.pushRule = function(params) {
			if (Array.isArray(params.next)) for (var i = 0; i < params.next.length; i++) params.next[i] = this.embeddedRulePrefix + params.next[i];
			return {
				regex: params.regex,
				onMatch: function(value, currentState, stack, line) {
					if (stack.length === 0) stack.push(currentState);
					if (Array.isArray(params.next)) for (var i$1 = 0; i$1 < params.next.length; i$1++) stack.push(params.next[i$1]);
					else stack.push(params.next);
				...
				set next(next) {
					if (!Array.isArray(params.next)) params.next = next;
				},
				get token() {
					return params.token;
				}
		...
		this.popRule = function(params) {
			if (params.next) params.next = this.embeddedRulePrefix + params.next;
			return {
				regex: params.regex,
				onMatch: function(value, currentState, stack, line) {
					stack.pop();
					if (params.next) {
						...
						this.next = stack[stack.length - 1];
					} else this.next = stack.length > 1 ? stack[stack.length - 1] : stack.pop();
					return params.token;
				}
			};
		};
	}).call(CsoundPreprocessorHighlightRules$1.prototype);
	exports.CsoundPreprocessorHighlightRules = CsoundPreprocessorHighlightRules$1;
}));
var require_csound_score_highlight_rules = /* @__PURE__ */ __commonJSMin(((exports) => {
	var oop = require_oop();
...
export { require_csound_preprocessor_highlight_rules as n, require_csound_score_highlight_rules as t };

...
`, "autocompletion.css", false);
  ...
  class ServiceManager {
    constructor(ctx) {
      ...
      this.ctx = ctx;
      let doValidation = async (document2, servicesInstances) => {
        var _a;
        servicesInstances != null ? servicesInstances : servicesInstances = this.getServicesInstances(document2.uri);
    ...
    removeDocument(document2) {
      let services = this.getServicesInstances(document2.uri);
      if (services.length > 0) {
        services.forEach((el) => el.removeDocument(document2));
        delete this.$sessionIDToMode[document2.uri];
      }
    }
    ...
    static for(servers, options) {
      ...
      if (servers instanceof Array) {
        servers.forEach((serverData, index) => {
          var _a2;
          serviceManager.registerServer((_a2 = serverData.serviceName) != null ? _a2 : "server" + index, serverData);
        });
      } else {
        serviceManager.registerServer((_a = servers.serviceName) != null ? _a : "server", servers);
      }
      return LanguageProvider.create(client, options);
    }
  }
  exports2.AceLanguageClient = AceLanguageClient;
